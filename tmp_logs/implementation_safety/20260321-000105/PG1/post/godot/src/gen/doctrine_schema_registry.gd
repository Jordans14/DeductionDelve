...
const FALLBACK_GOVERNANCE_SCHEMA := {
	...
	"semantic_lock_registry_required_fields": ["schema_name", "schema_version", "registry_id", "constitution_hash", "constitution_id", "tuple_records"],
	"semantic_lock_tuple_required_fields": [
		"tuple_id",
		"subject_class",
		"owner_lane",
...
const FALLBACK_EXPERIMENT_FAMILIES: Array[Dictionary] = [
	...
	},
	{
		...
		"recurrence_weight": 1,
		"public_lines": [
			"Older accounts are starting to show their newer layers without settling the older ones."
		],
		"hypothesis": {
			"hypothesis_id": "hyp_palimpsest",
			"domain": "memory",
			"thesis": "Layered archive memory stays legible when contradiction exposes newer readings without erasing older traces.",
			"confidence": 1,
			"target_layers": ["archive", "framing", "continuity"],
			"target_populations": ["archive readers", "legend keepers"],
			"supporting_evidence_ids": ["hybrid_lineage", "split_record"],
			"contradicting_evidence_ids": ["closed_case"],
			"open_branches": [],
			"persistence_state": "archival",
			"dormancy_state": "archival",
		...
		"experiment": {
			"experiment_id": "exp_palimpsest",
			"program_id": "palimpsest_program",
			"topology_type": "synthesis",
			"target": "archive",
			...
			"compile_outputs": {
				"archive_framing_bias": {
					"lines": [
						"Archive retellings are starting to show older memory layers beneath newer conclusions."
					],
					"emphasis_tags": ["memory fidelity", "layered archive"]
				},
				"public_activation": {
					"surface_lines": [
						"Older accounts are starting to show their newer layers without settling the older ones."
	...
	},
	{
		...
		"recurrence_weight": 1,
		"public_lines": [
			"What is missing from the record is starting to shape how the record gets read."
		],
		"hypothesis": {
			"hypothesis_id": "hyp_negative_space",
			"domain": "negative_space",
			"thesis": "Absence becomes legible when missing verification classes start shaping how archive cases are named.",
			"confidence": 1,
			"target_layers": ["ontology", "archive", "continuity"],
			"target_populations": ["taxonomy keepers", "archive readers"],
			"supporting_evidence_ids": ["classification_note", "residue_trace"],
			"contradicting_evidence_ids": ["clean_verification"],
			"open_branches": [],
			"persistence_state": "dormant",
			"dormancy_state": "dormant",
		...
		"experiment": {
			"experiment_id": "exp_negative_space",
			"program_id": "negative_space_program",
			"topology_type": "branching",
			"target": "ontology",
			"axis": "ambiguity",
			"stressor": "classification_drift",
			...
			"compile_outputs": {
				...
				"archive_framing_bias": {
					"lines": [
						"Archive framing is starting to register what missing classes are doing to settled records."
					],
					"emphasis_tags": ["absence pressure", "classification drift"]
				},
				"public_activation": {
					"surface_lines": [
						"What is missing from the record is starting to shape how the record gets read."
	...
	},
	{
		...
		"recurrence_weight": 1,
		"public_lines": [
			"People are starting to recognize when old echoes matter before anyone can fully name why."
		],
		"hypothesis": {
			"hypothesis_id": "hyp_echo_literacy",
			"domain": "memory",
			"thesis": "Echo literacy rises when archive residue stays meaningful across continuity without collapsing into closure.",
			"confidence": 2,
			"target_layers": ["archive", "framing", "continuity"],
			"target_populations": ["archive readers", "legend keepers"],
			"supporting_evidence_ids": ["legend_echo", "residue_trace"],
			"contradicting_evidence_ids": ["closed_case"],
			"open_branches": [],
			"persistence_state": "rare",
			"dormancy_state": "rare",
		...
		"experiment": {
			"experiment_id": "exp_echo_literacy",
			"program_id": "echo_literacy_program",
			"topology_type": "synthesis",
			"target": "continuity",
			"axis": "curiosity",
			"stressor": "archive_echo",
			...
			"compile_outputs": {
				...
				"archive_framing_bias": {
					"lines": [
						"Archive echoes are starting to matter before anyone can settle what they prove."
					],
					"emphasis_tags": ["echo residue", "memory pressure"]
				},
				"public_activation": {
					"surface_lines": [
						"People are starting to recognize when old echoes matter before anyone can fully name why."
	...
	{
		"id": "contraband_lite",
		"label": "Contraband-lite",
		"domain": "public_argument",
		"state": "rare",
		"target_layers": ["pressure", "archive", "framing"],
		"recurrence_weight": 1,
		"public_lines": [
			"Unofficial readings are starting to circulate, but they still read as provisional."
		],
		"hypothesis": {
			"hypothesis_id": "hyp_contraband_lite",
			"domain": "public_argument",
			"thesis": "Provisional public readings circulate when legitimacy formation outruns settled verification.",
			"confidence": 1,
			"target_layers": ["pressure", "archive", "framing"],
			"target_populations": ["public witnesses", "rumor keepers"],
			"supporting_evidence_ids": ["split_record", "witness_dispute"],
		...
		"experiment": {
			"experiment_id": "exp_contraband_lite",
			"program_id": "contraband_lite_program",
			"topology_type": "branching",
			"target": "framing",
...
static func _validate_constitution_schema(schema: Dictionary) -> Array[String]:
	...
	_require_values(
		_string_array(schema.get("semantic_lock_required_fields", [])),
		["schema_name", "schema_version", "registry_id", "constitution_hash", "constitution_id", "tuple_records"],
		"constitution_schema.json semantic_lock_required_fields",
		failures
	)
	return failures
...
static func _validate_governance_schema(schema: Dictionary) -> Array[String]:
	var failures := _validate_basic_schema(schema, "GovernanceState", [
		...
		"semantic_lock_registry_required_fields",
		"semantic_lock_tuple_required_fields",
	...
	_require_values(
		_string_array(schema.get("semantic_lock_registry_required_fields", [])),
		["schema_name", "schema_version", "registry_id", "constitution_hash", "constitution_id", "tuple_records"],
		"GovernanceState semantic_lock_registry_required_fields",
		failures
	)
	_require_values(
		_string_array(schema.get("semantic_lock_tuple_required_fields", [])),
		["tuple_id", "subject_class", "owner_lane", "canonical_source_owner", "canonical_source_fields", "digest_fields", "allowed_consumers", "visibility_class", "derivation_policy", "failure_codes"],
		"GovernanceState semantic_lock_tuple_required_fields",
		failures
	)
