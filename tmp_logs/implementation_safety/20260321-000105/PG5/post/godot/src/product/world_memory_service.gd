...
static func apply_run(world_memory: Dictionary, run_context: Dictionary) -> Dictionary:
	...
	current["effect_validation_records"] = Array(Dictionary(run_context.get("diagnostics", {})).get("effect_validation_records", [])).duplicate(true)
	current["delvemind_governance_contract"] = Dictionary(
		Dictionary(
			Dictionary(run_context.get("profile", {})).get("delvemind_experiment_state", {})
		).get("governance_contract", {})
	).duplicate(true)
	current["localized_attractor_surface"] = Dictionary(Dictionary(run_context.get("diagnostics", {})).get("localized_attractor_surface", {})).duplicate(true)
...
static func _update_artifact_consequence_state(world_memory: Dictionary, run_context: Dictionary) -> void:
	...
	var diagnostics: Dictionary = Dictionary(run_context.get("diagnostics", {}))
	var outcome_summary: Dictionary = Dictionary(run_record.get("outcome_summary", {}))
	var artifact_consequence_ref: Dictionary = Dictionary(Dictionary(diagnostics.get("consequence_bridge", {})).get("artifact_consequence_ref", outcome_summary.get("artifact_consequence_ref", {}))).duplicate(true)
	if outcome_summary.is_empty() and artifact_consequence_ref.is_empty():
		world_memory["artifact_consequence_state"] = artifact_consequence_state
		return
	artifact_consequence_state["artifact_consequence_version"] = int(artifact_consequence_ref.get("artifact_consequence_version", outcome_summary.get("artifact_consequence_version", artifact_consequence_state.get("artifact_consequence_version", 0))))
	artifact_consequence_state["consequence_event_family"] = str(artifact_consequence_ref.get("consequence_event_family", outcome_summary.get("consequence_event_family", artifact_consequence_state.get("consequence_event_family", "")))).strip_edges()
	artifact_consequence_state["burden_band"] = str(artifact_consequence_ref.get("burden_band", outcome_summary.get("burden_band", artifact_consequence_state.get("burden_band", "")))).strip_edges()
	artifact_consequence_state["valuation_band"] = str(artifact_consequence_ref.get("valuation_band", outcome_summary.get("valuation_band", artifact_consequence_state.get("valuation_band", "")))).strip_edges()
	artifact_consequence_state["return_consequence_state"] = str(artifact_consequence_ref.get("return_consequence_state", outcome_summary.get("return_consequence_state", artifact_consequence_state.get("return_consequence_state", "")))).strip_edges()
	artifact_consequence_state["market_regime_id"] = str(artifact_consequence_ref.get("market_regime_id", outcome_summary.get("market_regime_id", artifact_consequence_state.get("market_regime_id", "")))).strip_edges()
	artifact_consequence_state["market_carrier_risk_band"] = str(artifact_consequence_ref.get("market_carrier_risk_band", outcome_summary.get("market_carrier_risk_band", artifact_consequence_state.get("market_carrier_risk_band", "")))).strip_edges()
	artifact_consequence_state["public_consequence_tags"] = _merge_limited(
		...
		Array(artifact_consequence_ref.get("public_consequence_tags", outcome_summary.get("public_consequence_tags", []))),
		8
	)
	artifact_consequence_state["artifact_consequence_ref"] = artifact_consequence_ref.duplicate(true)
	var summary_line := _first_string([
		"%s under %s" % [
			str(artifact_consequence_ref.get("return_consequence_state", outcome_summary.get("return_consequence_state", ""))).replace("_", " "),
			str(artifact_consequence_ref.get("valuation_band", outcome_summary.get("valuation_band", ""))).replace("_", " ")
		],
...
static func _update_social_consequence_state(world_memory: Dictionary, run_context: Dictionary) -> void:
	...
	var run_record: Dictionary = Dictionary(run_context.get("run_record", {}))
	var diagnostics: Dictionary = Dictionary(run_context.get("diagnostics", {}))
	var social_consequence_ref: Dictionary = Dictionary(Dictionary(diagnostics.get("consequence_bridge", {})).get("social_consequence_ref", {})).duplicate(true)
	var version := int(social_consequence_ref.get("social_consequence_version", run_record.get("social_consequence_version", diagnostics.get("social_consequence_version", 0))))
	if version <= 0:
	...
	social_consequence_state["public_evidence_tags"] = _merge_limited(
		...
		_string_array(social_consequence_ref.get("public_evidence_tags", run_record.get("public_evidence_tags", diagnostics.get("public_evidence_tags", [])))),
		8
	)
	social_consequence_state["witness_pressure"] = str(social_consequence_ref.get("witness_pressure", run_record.get("witness_pressure", diagnostics.get("witness_pressure", social_consequence_state.get("witness_pressure", ""))))).strip_edges()
	social_consequence_state["counterfeit_pressure"] = str(social_consequence_ref.get("counterfeit_pressure", run_record.get("counterfeit_pressure", diagnostics.get("counterfeit_pressure", social_consequence_state.get("counterfeit_pressure", ""))))).strip_edges()
	social_consequence_state["relationship_pressure"] = str(social_consequence_ref.get("relationship_pressure", run_record.get("relationship_pressure", diagnostics.get("relationship_pressure", social_consequence_state.get("relationship_pressure", ""))))).strip_edges()
	social_consequence_state["blame_surface_tags"] = _merge_limited(
		Array(social_consequence_state.get("blame_surface_tags", [])),
		_string_array(social_consequence_ref.get("blame_surface_tags", run_record.get("blame_surface_tags", diagnostics.get("blame_surface_tags", [])))),
		8
	)
	...
	social_consequence_state["social_consequence_ref"] = social_consequence_ref.duplicate(true)
...
static func _update_encounter_apex_consequence_state(world_memory: Dictionary, run_context: Dictionary) -> void:
	...
	var diagnostics: Dictionary = Dictionary(run_context.get("diagnostics", {}))
	var encounter_apex_ref: Dictionary = Dictionary(Dictionary(diagnostics.get("consequence_bridge", {})).get("encounter_apex_ref", {})).duplicate(true)
	var version := int(encounter_apex_ref.get("encounter_apex_consequence_version", run_record.get("encounter_apex_consequence_version", local_aftermath.get("encounter_apex_consequence_version", 0))))
	if version <= 0:
	...
	encounter_apex_consequence_state["encounter_resolution_state"] = str(encounter_apex_ref.get("encounter_resolution_state", run_record.get("encounter_resolution_state", local_aftermath.get("encounter_resolution_state", encounter_apex_consequence_state.get("encounter_resolution_state", ""))))).strip_edges()
	encounter_apex_consequence_state["apex_resolution_state"] = str(encounter_apex_ref.get("apex_resolution_state", run_record.get("apex_resolution_state", local_aftermath.get("apex_resolution_state", encounter_apex_consequence_state.get("apex_resolution_state", ""))))).strip_edges()
	encounter_apex_consequence_state["anchored_pressures"] = _merge_limited(
		...
		_string_array(encounter_apex_ref.get("anchored_pressures", run_record.get("anchored_pressures", local_aftermath.get("anchored_pressures", [])))),
		8
	)
	encounter_apex_consequence_state["consequence_classes"] = _merge_limited(
		...
		_string_array(encounter_apex_ref.get("consequence_classes", run_record.get("consequence_classes", local_aftermath.get("consequence_classes", [])))),
		8
	)
	...
	encounter_apex_consequence_state["encounter_apex_ref"] = encounter_apex_ref.duplicate(true)
	var summary_line := ""
...
static func _update_lifecycle_registry(world_memory: Dictionary, run_context: Dictionary) -> void:
	...
	var constitution_summary: Dictionary = Dictionary(run_record.get("expedition_constitution_summary", {}))
	var lifecycle_ref: Dictionary = Dictionary(Dictionary(diagnostics.get("consequence_bridge", {})).get("lifecycle_ref", {})).duplicate(true)
	...
	if Array(incoming_registry.get("families", [])).is_empty():
		incoming_registry = _normalize_lifecycle_registry({
			"families": [{
				"family_id": _first_string(_string_array(lifecycle_ref.get("active_regime_ids", constitution_summary.get("active_regime_ids", []))), "market_balanced_exchange"),
				"family_kind": "market",
				"source_id": _first_string(_string_array(lifecycle_ref.get("active_regime_ids", constitution_summary.get("active_regime_ids", []))), "market_balanced_exchange"),
				"state": "emerging",
				"heat": 1,
				"saturation": 1,
				"strain": 0,
			...
			"active_state_ids": _string_array(lifecycle_ref.get("lifecycle_state_ids", constitution_summary.get("lifecycle_state_ids", []))),
			"lines": _string_array(lifecycle_ref.get("lifecycle_lines", constitution_summary.get("lifecycle_lines", [])))
		})
	var family_index := {}
	...
	lifecycle_registry["lifecycle_ref"] = lifecycle_ref.duplicate(true)
