...
static func normalize(state: Dictionary) -> Dictionary:
	...
	current["governance_contract"] = _normalize_governance_contract(Dictionary(current.get("governance_contract", {})))
...
static func validate_state(state: Dictionary) -> Array[String]:
	...
	var governance_contract: Dictionary = Dictionary(state.get("governance_contract", {}))
	if governance_contract.is_empty():
		failures.append("DelveMindExperimentState missing governance_contract")
	else:
		var normalized_contract := _normalize_governance_contract(governance_contract)
		for key in [
			"runtime_authority",
			...
			"contextual_non_final_observation_required",
			"persistence_report_safe_only",
			"uncertainty_preservation_required",
			"bias_mitigation_required"
		]:
			if governance_contract.get(key) != normalized_contract.get(key):
				failures.append("DelveMind governance_contract must preserve %s" % key)
		if _string_array(governance_contract.get("forbidden_runtime_fields", [])) != _string_array(normalized_contract.get("forbidden_runtime_fields", [])):
			failures.append("DelveMind governance_contract must preserve forbidden_runtime_fields")
		if _string_array(governance_contract.get("forbidden_outputs", [])) != _string_array(normalized_contract.get("forbidden_outputs", [])):
			failures.append("DelveMind governance_contract must preserve forbidden_outputs")
		if int(governance_contract.get("public_lines_max", 0)) != int(normalized_contract.get("public_lines_max", 6)):
			failures.append("DelveMind governance_contract must preserve public_lines_max")
		if int(governance_contract.get("operator_lines_max", 0)) != int(normalized_contract.get("operator_lines_max", 6)):
			failures.append("DelveMind governance_contract must preserve operator_lines_max")
...
static func _default_shell() -> Dictionary:
	return {
		...
		"simulation_chambers": {},
		"governance_contract": _default_governance_contract(),
		"history_lines": [],
		"foundational_ids": [],
		"archival_ids": [],
		"lineage_index": {},
...
static func _default_governance_contract() -> Dictionary:
	return {
		...
		"forbidden_runtime_fields": RUNTIME_FORBIDDEN_FIELDS.duplicate(true),
...
static func _normalize_governance_contract(raw: Dictionary) -> Dictionary:
	var current := _default_governance_contract()
	for key in raw.keys():
		current[key] = raw[key]
	current["schema_name"] = "DelveMindGovernanceContract"
	current["schema_version"] = 1
	current["runtime_authority"] = false
	current["hidden_targeting_authority"] = false
	current["moral_classification_allowed"] = false
	current["player_essence_typing_allowed"] = false
	current["bounded_inference_required"] = true
	current["contextual_non_final_observation_required"] = true
	current["persistence_report_safe_only"] = true
	current["uncertainty_preservation_required"] = true
	current["bias_mitigation_required"] = true
	current["allowed_output_layers"] = _string_array(current.get("allowed_output_layers", _default_governance_contract().get("allowed_output_layers", [])))
	current["forbidden_runtime_fields"] = _string_array(current.get("forbidden_runtime_fields", RUNTIME_FORBIDDEN_FIELDS))
	current["forbidden_outputs"] = _string_array(current.get("forbidden_outputs", _default_governance_contract().get("forbidden_outputs", [])))
	current["allowed_subject_domains"] = _string_array(current.get("allowed_subject_domains", _default_governance_contract().get("allowed_subject_domains", [])))
	current["observation_status_domain"] = _string_array(current.get("observation_status_domain", _default_governance_contract().get("observation_status_domain", [])))
	current["public_lines_max"] = 6
	current["operator_lines_max"] = 6
	current["failure_codes"] = _string_array(current.get("failure_codes", _default_governance_contract().get("failure_codes", [])))
	return current
