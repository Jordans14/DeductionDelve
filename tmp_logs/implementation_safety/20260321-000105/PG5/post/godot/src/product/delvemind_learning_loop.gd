...
const FORBIDDEN_LINE_FRAGMENTS := [
	"guilt_score",
	"role_truth",
	"artifact_truth",
	"runtime_targeting",
...
static func validate_learning_state(learning_state: Dictionary, hypotheses: Dictionary = {}, experiments: Dictionary = {}) -> Array[String]:
	...
	if _string_array(learning_state.get("public_lines", [])).size() > MAX_TRACE_LINES:
		failures.append("DelveMindLearningState public_lines must stay within the governance line budget")
	if _string_array(learning_state.get("operator_lines", [])).size() > MAX_TRACE_LINES:
		failures.append("DelveMindLearningState operator_lines must stay within the governance line budget")
	for fragment in FORBIDDEN_LINE_FRAGMENTS:
		for public_line in _string_array(learning_state.get("public_lines", [])):
			if public_line.to_lower().find(fragment) != -1:
				failures.append("DelveMindLearningState public_lines must not contain forbidden fragment %s" % fragment)
				break
		for operator_line in _string_array(learning_state.get("operator_lines", [])):
			if operator_line.to_lower().find(fragment) != -1:
				failures.append("DelveMindLearningState operator_lines must not contain forbidden fragment %s" % fragment)
				break
