...
static func build_world_aftermath_records(run_context: Dictionary) -> Array[Dictionary]:
	...
	var encounter_apex_ref: Dictionary = Dictionary(consequence_bridge.get("encounter_apex_ref", {})).duplicate(true)
	...
	if aftermath_refs.is_empty() and not local_aftermath.is_empty():
		aftermath_refs = [{
			...
			"schema_version": 1,
			"encounter_apex_consequence_version": int(encounter_apex_ref.get("encounter_apex_consequence_version", local_aftermath.get("encounter_apex_consequence_version", 0))),
			...
			"encounter_resolution_state": str(encounter_apex_ref.get("encounter_resolution_state", local_aftermath.get("encounter_resolution_state", ""))).strip_edges(),
			"apex_resolution_state": str(encounter_apex_ref.get("apex_resolution_state", local_aftermath.get("apex_resolution_state", ""))).strip_edges(),
			"anchored_pressures": Array(encounter_apex_ref.get("anchored_pressures", local_aftermath.get("anchored_pressures", []))).duplicate(true),
			"consequence_classes": Array(encounter_apex_ref.get("consequence_classes", local_aftermath.get("consequence_classes", []))).duplicate(true),
			...
			"aftermath_consequence_refs": Array(local_aftermath.get("aftermath_consequence_refs", encounter_apex_ref.get("world_aftermath_ref_ids", []))).duplicate(true)
		}]
	...
	for aftermath_ref_raw in aftermath_refs:
		...
		result.append({
			...
			"schema_version": 1,
			"encounter_apex_consequence_version": int(aftermath_ref.get("encounter_apex_consequence_version", encounter_apex_ref.get("encounter_apex_consequence_version", local_aftermath.get("encounter_apex_consequence_version", 0)))),
...
static func apply_post_run_extensions(world_memory: Dictionary, run_context: Dictionary) -> Dictionary:
	...
	var diagnostics: Dictionary = Dictionary(run_context.get("diagnostics", {}))
	var consequence_bridge: Dictionary = Dictionary(diagnostics.get("consequence_bridge", {})).duplicate(true)
	var artifact_consequence_ref: Dictionary = Dictionary(consequence_bridge.get("artifact_consequence_ref", {})).duplicate(true)
	var encounter_apex_ref: Dictionary = Dictionary(consequence_bridge.get("encounter_apex_ref", {})).duplicate(true)
	...
	current["artifact_consequence_surface"] = _normalize_artifact_consequence_surface({
		"artifact_consequence_version": int(artifact_consequence_ref.get("artifact_consequence_version", outcome_summary.get("artifact_consequence_version", 0))),
		"consequence_event_family": str(artifact_consequence_ref.get("consequence_event_family", outcome_summary.get("consequence_event_family", ""))).strip_edges(),
		"burden_band": str(artifact_consequence_ref.get("burden_band", outcome_summary.get("burden_band", ""))).strip_edges(),
		"valuation_band": str(artifact_consequence_ref.get("valuation_band", outcome_summary.get("valuation_band", ""))).strip_edges(),
		"return_consequence_state": str(artifact_consequence_ref.get("return_consequence_state", outcome_summary.get("return_consequence_state", ""))).strip_edges(),
		"market_regime_id": str(artifact_consequence_ref.get("market_regime_id", outcome_summary.get("market_regime_id", market_regime_id))).strip_edges(),
		"market_carrier_risk_band": str(artifact_consequence_ref.get("market_carrier_risk_band", outcome_summary.get("market_carrier_risk_band", constitution_summary.get("market_carrier_risk_band", "")))).strip_edges(),
		"public_consequence_tags": _string_array(artifact_consequence_ref.get("public_consequence_tags", outcome_summary.get("public_consequence_tags", []))),
		...
		"lines": _string_array(
			...
			if not str(artifact_consequence_ref.get("return_consequence_state", outcome_summary.get("return_consequence_state", ""))).strip_edges().is_empty() or not str(artifact_consequence_ref.get("valuation_band", outcome_summary.get("valuation_band", ""))).strip_edges().is_empty()
			else []
		)
	})
	current["encounter_apex_consequence_surface"] = _normalize_encounter_apex_consequence_surface({
		"encounter_apex_consequence_version": int(encounter_apex_ref.get("encounter_apex_consequence_version", run_record.get("encounter_apex_consequence_version", Dictionary(run_record.get("local_aftermath", {})).get("encounter_apex_consequence_version", 0)))),
		"encounter_resolution_state": str(encounter_apex_ref.get("encounter_resolution_state", run_record.get("encounter_resolution_state", Dictionary(run_record.get("local_aftermath", {})).get("encounter_resolution_state", "")))).strip_edges(),
		"apex_resolution_state": str(encounter_apex_ref.get("apex_resolution_state", run_record.get("apex_resolution_state", Dictionary(run_record.get("local_aftermath", {})).get("apex_resolution_state", "")))).strip_edges(),
		"anchored_pressures": _string_array(encounter_apex_ref.get("anchored_pressures", run_record.get("anchored_pressures", Dictionary(run_record.get("local_aftermath", {})).get("anchored_pressures", [])))),
		"consequence_classes": _string_array(encounter_apex_ref.get("consequence_classes", run_record.get("consequence_classes", Dictionary(run_record.get("local_aftermath", {})).get("consequence_classes", [])))),
		...
		"lines": _string_array(
			...
			if int(encounter_apex_ref.get("encounter_apex_consequence_version", run_record.get("encounter_apex_consequence_version", Dictionary(run_record.get("local_aftermath", {})).get("encounter_apex_consequence_version", 0)))) > 0
			else []
		)
	})
	...
	var interpretation_network: Dictionary = Dictionary(current.get("interpretation_network", {}))
	var localized_attractor_summary_lines := _slice_strings(
		_string_array(diagnostics.get("civilization_surface_lines", []))
		+ _string_array(diagnostics.get("cognitive_field_summary_lines", []))
		+ _string_array(diagnostics.get("review_surface_lines", [])),
		4
	)
	current["localized_attractor_surface"] = {
		...
		"market_regime_id": market_regime_id,
		"market_regime_family": str(constitution_summary.get("market_regime_family", "balanced")).strip_edges(),
		"active_regime_ids": active_regime_ids.duplicate(),
		...
		"contradiction_heat": clampi(int(interpretation_network.get("contradiction_heat", 0)), 0, 12),
		"activation_channels": _string_array(Dictionary(diagnostics.get("localized_attractor_surface", {})).get("activation_channels", [])),
		"safe_mode_active": bool(diagnostics.get("safe_mode_active", false)),
		"cognitive_dimensions": _string_array(diagnostics.get("cognitive_field_dimensions", [])),
		"artifact_consequence_tags": _string_array(Dictionary(current.get("artifact_consequence_surface", {})).get("public_consequence_tags", [])),
		"social_pressure_tags": _string_array(diagnostics.get("public_evidence_tags", [])) + _string_array(diagnostics.get("blame_surface_tags", [])),
		"encounter_apex_tags": _string_array(Dictionary(current.get("encounter_apex_consequence_surface", {})).get("world_aftermath_tags", [])) + _string_array(Dictionary(current.get("encounter_apex_consequence_surface", {})).get("local_aftermath_tags", [])),
		"residue_ids": _pluck_ids(_normalize_residue(Array(current.get("residue_records", []))), "residue_id"),
		"summary_lines": localized_attractor_summary_lines,
		"source_refs": ["world_memory", "diagnostics", "constitution_summary", "civilization_state"]
	}
