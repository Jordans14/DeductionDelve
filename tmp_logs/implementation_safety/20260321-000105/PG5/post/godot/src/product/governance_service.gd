...
static func evaluate_planning_state(governance_state: Dictionary, world_model: Dictionary, theory_surface: Dictionary, civilization_surface: Dictionary, contradiction_packet: Dictionary, cookbook_state: Dictionary) -> Dictionary:
	...
	current["localized_attractor_surface"] = {
		...
		"surface_id": "localized_attractor_%s" % JSON.stringify({
			"market_regime_ids": _string_array(civilization_surface.get("market_regime_ids", [])),
			"theory_ids": _string_array(theory_surface.get("theory_ids", [])),
			"world_mutation_ids": _string_array(civilization_surface.get("world_mutation_ids", []))
		}).md5_text().substr(0, 12),
		"constitution_hash": str(Dictionary(world_model.get("session_model", {})).get("constitution_hash", "")).strip_edges(),
		"run_seed": maxi(int(Dictionary(world_model.get("session_model", {})).get("run_seed", 0)), 0),
		"market_regime_id": _first_string(civilization_surface.get("market_regime_ids", []), ""),
		"market_regime_family": str(Dictionary(world_model.get("economy_model", {})).get("prestige_climate", "")).strip_edges(),
		...
		"faction_ids": _string_array(civilization_surface.get("faction_ids", [])),
		"theory_ids": _string_array(theory_surface.get("theory_ids", [])),
		"theory_statuses": _string_array(theory_surface.get("statuses", [])),
		"world_mutation_ids": _string_array(civilization_surface.get("world_mutation_ids", [])),
		"contradiction_heat": clampi(int(contradiction_packet.get("contradiction_heat", contradiction_records.size())), 0, 12),
		"activation_channels": active_unique.duplicate(),
		"safe_mode_active": safe_mode_enabled,
		"cognitive_dimensions": _string_array(Dictionary(world_model.get("localized_attractor_surface", {})).get("cognitive_dimensions", [])),
		"artifact_consequence_tags": _string_array(civilization_surface.get("artifact_consequence_tags", [])),
		"social_pressure_tags": _string_array(Dictionary(world_model.get("localized_attractor_surface", {})).get("social_pressure_tags", [])),
		"encounter_apex_tags": _string_array(civilization_surface.get("encounter_apex_consequence_tags", [])),
		"residue_ids": _string_array(Dictionary(world_model.get("localized_attractor_surface", {})).get("residue_ids", [])),
		"summary_lines": _slice_strings(
			_string_array(civilization_surface.get("lines", []))
			+ _summary_lines_for_reports(contradiction_records, 2)
			+ _string_array(Dictionary(world_model.get("localized_attractor_surface", {})).get("summary_lines", [])),
			4
		),
		"source_refs": ["world_model", "theory_surface", "civilization_surface", "contradiction_packet"]
	}
...
static func apply_post_run(governance_state: Dictionary, run_record: Dictionary, diagnostics: Dictionary, frame: Dictionary, constitution_summary: Dictionary) -> Dictionary:
	...
	current["localized_attractor_surface"] = {
		...
		"surface_id": "localized_attractor_%s" % JSON.stringify({
			...
			"theory_ids": _string_array(diagnostics.get("theory_ids", [])),
			"world_mutation_ids": _string_array(diagnostics.get("world_mutation_ids", []))
		}).md5_text().substr(0, 12),
		"constitution_hash": str(constitution_summary.get("constitution_hash", "")).strip_edges(),
		...
		"active_regime_ids": _string_array(constitution_summary.get("active_regime_ids", [])),
		"lifecycle_state_ids": _string_array(constitution_summary.get("lifecycle_state_ids", [])),
		"faction_ids": _string_array(diagnostics.get("civilization_faction_ids", [])),
		"theory_ids": _string_array(diagnostics.get("theory_ids", [])),
		"theory_statuses": _string_array(diagnostics.get("theory_statuses", [])),
		"world_mutation_ids": _string_array(diagnostics.get("world_mutation_ids", [])),
		"contradiction_heat": clampi(int(Dictionary(diagnostics.get("localized_attractor_surface", {})).get("contradiction_heat", contradiction_records.size())), 0, 12),
		"activation_channels": _string_array(activation_state.get("active_channels", [])),
		"safe_mode_active": bool(safe_mode_state.get("enabled", false)),
		"cognitive_dimensions": _string_array(diagnostics.get("cognitive_field_dimensions", [])),
		"artifact_consequence_tags": _string_array(diagnostics.get("public_consequence_tags", [])),
		"social_pressure_tags": _string_array(diagnostics.get("public_evidence_tags", [])) + _string_array(diagnostics.get("blame_surface_tags", [])),
		"encounter_apex_tags": _string_array(diagnostics.get("world_aftermath_tags", [])) + _string_array(diagnostics.get("local_aftermath_tags", [])),
		"residue_ids": _string_array(Dictionary(diagnostics.get("localized_attractor_surface", {})).get("residue_ids", [])),
		"summary_lines": _slice_strings(
			_string_array(diagnostics.get("civilization_surface_lines", []))
			+ _string_array(diagnostics.get("cognitive_field_summary_lines", []))
			+ _summary_lines_for_reports(contradiction_records, 1),
			4
		),
		"source_refs": ["constitution_summary", "diagnostics", "governance_state", "run_record"]
	}
	return normalize(current)
