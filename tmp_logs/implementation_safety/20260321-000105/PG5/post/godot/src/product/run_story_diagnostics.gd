...
static func analyze(run_record: Dictionary) -> Dictionary:
	...
	var truth_resolution_envelope: Dictionary = Dictionary(Dictionary(run_record.get("forensic_bundle", {})).get("truth_resolution_envelope", run_record.get("truth_resolution_envelope", {}))).duplicate(true)
	var truth_resolution_envelope_id := str(truth_resolution_envelope.get("envelope_id", "")).strip_edges()
	var truth_resolution_authority_epoch := str(truth_resolution_envelope.get("authority_epoch", "")).strip_edges()
	var truth_resolution_owner_lane := str(truth_resolution_envelope.get("owner_lane", "")).strip_edges()
	var truth_resolution_resolution_class := str(truth_resolution_envelope.get("resolution_class", "")).strip_edges()
	var truth_resolution_visibility_class := str(truth_resolution_envelope.get("visibility_class", "")).strip_edges()
	var truth_resolution_contradiction_disposition := str(truth_resolution_envelope.get("contradiction_disposition", "")).strip_edges()
	var truth_resolution_source_constitution_hash := str(truth_resolution_envelope.get("source_constitution_hash", "")).strip_edges()
	var truth_resolution_source_digest := str(truth_resolution_envelope.get("source_digest", "")).strip_edges()
	var truth_resolution_public_summary_refs := _take_unique(_string_array(truth_resolution_envelope.get("public_summary_refs", [])), 8)
	var truth_resolution_operator_refs := _take_unique(_string_array(truth_resolution_envelope.get("operator_refs", [])), 8)
	...
	var constitution_hash := str(
		run_record.get(
			"constitution_hash",
			expedition_constitution_summary.get("constitution_hash", truth_resolution_source_constitution_hash)
		)
	).strip_edges()
	var combo_contract_ref: Dictionary = Dictionary(gameplay_model.get("combo_contract_ref", run_record.get("combo_contract_ref", {}))).duplicate(true)
	if combo_contract_ref.is_empty():
		combo_contract_ref = {
			...
			"private_entry_digest": JSON.stringify(combo_entries).md5_text() if not combo_entries.is_empty() else ""
		}
	var artifact_consequence_ref: Dictionary = Dictionary(outcome_summary.get("artifact_consequence_ref", {})).duplicate(true)
	if artifact_consequence_ref.is_empty():
		artifact_consequence_ref = {
	...
	var social_consequence_ref: Dictionary = Dictionary(run_record.get("social_consequence_ref", group_gameplay_model.get("social_consequence_ref", {}))).duplicate(true)
	if social_consequence_ref.is_empty():
		social_consequence_ref = {
			"social_consequence_version": social_consequence_version,
			"public_evidence_tags": public_evidence_tags.duplicate(),
			"witness_pressure": witness_pressure,
			"counterfeit_pressure": counterfeit_pressure,
			"relationship_pressure": relationship_pressure,
			"blame_surface_tags": blame_surface_tags.duplicate(),
			"private_evidence_digest": JSON.stringify(private_evidence_tags).md5_text() if not private_evidence_tags.is_empty() else ""
		}
	var encounter_apex_ref := {
		...
		"world_aftermath_ref_ids": _dict_values_to_string_array(world_aftermath_refs, "aftermath_id")
	}
	var lifecycle_ref := {
		"lifecycle_state_ids": _string_array(expedition_constitution_summary.get("lifecycle_state_ids", [])),
		"active_regime_ids": _string_array(expedition_constitution_summary.get("active_regime_ids", [])),
		"market_regime_id": market_regime_id,
		"market_regime_family": str(expedition_constitution_summary.get("market_regime_family", "")).strip_edges(),
		"market_regime_lines": _string_array(expedition_constitution_summary.get("market_regime_lines", [])),
		"lifecycle_lines": _string_array(expedition_constitution_summary.get("lifecycle_lines", []))
	}
	var onboarding_public_safe_ref := {
		"explanation_packet_lines": explanation_packet_lines.slice(0, 6),
		"review_surface_lines": review_surface_lines.slice(0, 6),
		"signal_budget_lines": signal_budget_lines.slice(0, 6),
		"legacy_track_id": "",
		"reentry_hook_id": ""
	}
	var consequence_bridge := {
		"schema_name": "ConsequenceBridge",
		"schema_version": 1,
		"bridge_id": "",
		"constitution_hash": constitution_hash,
		"combo_contract_ref": combo_contract_ref.duplicate(true),
		"artifact_consequence_ref": artifact_consequence_ref.duplicate(true),
		"social_consequence_ref": social_consequence_ref.duplicate(true),
		"encounter_apex_ref": encounter_apex_ref.duplicate(true),
		"lifecycle_ref": lifecycle_ref.duplicate(true),
		"onboarding_public_safe_ref": onboarding_public_safe_ref.duplicate(true),
		"failure_codes": [
			"consequence_event_family_mismatch",
			"public_private_projection_leak",
			"onboarding_reinterpretation"
		]
	}
	...
	var localized_attractor_surface := {
		"schema_name": "LocalizedAttractorSurface",
		"schema_version": 1,
		"surface_id": "localized_attractor_%s" % JSON.stringify({
			"constitution_hash": constitution_hash,
		...
		"constitution_hash": constitution_hash,
		"run_seed": maxi(int(run_record.get("seed", 0)), 0),
		"market_regime_id": market_regime_id,
		"market_regime_family": str(expedition_constitution_summary.get("market_regime_family", "")).strip_edges(),
		"active_regime_ids": _string_array(expedition_constitution_summary.get("active_regime_ids", [])),
		"lifecycle_state_ids": _string_array(expedition_constitution_summary.get("lifecycle_state_ids", [])),
		"faction_ids": civilization_faction_ids.duplicate(),
		"theory_ids": theory_ids.duplicate(),
		"theory_statuses": theory_statuses.duplicate(),
		"world_mutation_ids": world_mutation_ids.duplicate(),
		"contradiction_heat": clampi(maxi(theory_statuses.size() - 1, 0) + (1 if safe_mode_active else 0) + (1 if str(disclosure_review.get("contradiction_disposition", "stable_surface")).strip_edges() != "stable_surface" else 0), 0, 12),
		"activation_channels": activation_active_channels.duplicate(),
		"safe_mode_active": safe_mode_active,
		"cognitive_dimensions": cognitive_field_dimensions.duplicate(),
		"artifact_consequence_tags": public_consequence_tags.duplicate(),
		"social_pressure_tags": _take_unique(public_evidence_tags + blame_surface_tags, 8),
		"encounter_apex_tags": _take_unique(world_aftermath_tags + local_aftermath_tags, 8),
		"residue_ids": _take_unique(_dict_values_to_string_array(world_aftermath_refs, "aftermath_id") + lineage_registry_ids, 8),
		"summary_lines": _take_unique(civilization_surface_lines + cognitive_field_summary_lines + review_surface_lines, 4),
		"source_refs": ["expedition_constitution_summary", "consequence_bridge", "truth_resolution_envelope", "run_record"]
	}
	var delvemind_governance_contract := {
		"schema_name": "DelveMindGovernanceContract",
		"schema_version": 1,
		...
		"allowed_output_layers": ["compile_bias", "persistence_learning", "report_safe_summary", "operator_review"],
		"forbidden_runtime_fields": ["peer_ids", "runtime_state", "event_log", "physics_override", "legality_override", "artifact_truth_override", "runtime_ai_arbitration"],
		"forbidden_outputs": ["guilt_score", "role_truth", "artifact_truth", "runtime_targeting"],
		"allowed_subject_domains": ["authority", "trust", "ritual", "counterfeit", "taxonomy", "memory", "wonder", "public_fracture", "negative_space", "artifact_custody", "stewardship", "public_argument", "ritual_memory", "taxonomy_memory", "wonder_allocation"],
		"observation_status_domain": ["public_safe_summary", "traceable_archive_only", "pressure_trace", "constitution_trace"],
		"public_lines_max": 6,
	...
	var effect_validation_records: Array[Dictionary] = []
	var lifecycle_runtime_refs := _take_unique(_string_array(lifecycle_ref.get("lifecycle_state_ids", [])) + _string_array(lifecycle_ref.get("active_regime_ids", [])), 8)
	var lifecycle_report_refs := _take_unique(_string_array(lifecycle_ref.get("market_regime_lines", [])) + _string_array(lifecycle_ref.get("lifecycle_lines", [])), 8)
	effect_validation_records.append({
		"schema_name": "EffectValidationRecord",
		"schema_version": 1,
		"record_id": "effect_validation_lifecycle_%s" % str(run_record.get("seed", 0)),
		"constitution_hash": constitution_hash,
		"run_seed": maxi(int(run_record.get("seed", 0)), 0),
		"subject_kind": "lifecycle",
		"subject_id": _first(_string_array(lifecycle_ref.get("lifecycle_state_ids", [])), market_regime_id if not market_regime_id.is_empty() else "lifecycle_surface"),
		"ev_class": "ev4_lifecycle",
		"runtime_refs": lifecycle_runtime_refs,
		"report_refs": lifecycle_report_refs,
		"bundle_refs": _take_unique([truth_resolution_envelope_id] if not truth_resolution_envelope_id.is_empty() else [], 4),
		"persistence_refs": [],
		"reload_refs": [],
		"cashout_state": "passed" if not lifecycle_runtime_refs.is_empty() and not lifecycle_report_refs.is_empty() else "not_applicable",
		"failure_codes": [] if not lifecycle_runtime_refs.is_empty() and not lifecycle_report_refs.is_empty() else ["missing_lifecycle_cashout"],
		"summary_lines": ["lifecycle and regime carryover remain visible through the canonical public-safe surfaces"]
	})
	var cross_pillar_runtime_refs := _take_unique(
		[
			str(consequence_bridge.get("bridge_id", "")).strip_edges(),
			str(artifact_consequence_ref.get("consequence_event_family", "")).strip_edges(),
			str(combo_contract_ref.get("combo_contract_digest", "")).strip_edges(),
			market_regime_id
		] + theory_ids,
		8
	)
	var cross_pillar_report_refs := _take_unique(
		public_consequence_tags + public_evidence_tags + world_aftermath_tags + explanation_packet_lines,
		8
	)
	effect_validation_records.append({
		"schema_name": "EffectValidationRecord",
		"schema_version": 1,
		"record_id": "effect_validation_cross_pillar_%s" % str(run_record.get("seed", 0)),
		"constitution_hash": constitution_hash,
		"run_seed": maxi(int(run_record.get("seed", 0)), 0),
		"subject_kind": "cross_pillar",
		"subject_id": str(consequence_bridge.get("bridge_id", "")).strip_edges(),
		"ev_class": "evx_cross_pillar",
		"runtime_refs": cross_pillar_runtime_refs,
		"report_refs": cross_pillar_report_refs,
		"bundle_refs": _take_unique(
			([truth_resolution_envelope_id] if not truth_resolution_envelope_id.is_empty() else [])
			+ ([forensic_bundle_digest] if not forensic_bundle_digest.is_empty() else []),
			4
		),
		"persistence_refs": [],
		"reload_refs": [],
		"cashout_state": "passed" if not str(consequence_bridge.get("bridge_id", "")).strip_edges().is_empty() and not cross_pillar_report_refs.is_empty() else "failed",
		"failure_codes": [] if not str(consequence_bridge.get("bridge_id", "")).strip_edges().is_empty() and not cross_pillar_report_refs.is_empty() else ["missing_cross_pillar_cashout"],
		"summary_lines": ["cross-pillar consequence meaning remains aligned across combo, artifact, social, aftermath, and lifecycle carryover"]
	})
	...
	return {
		...
		"truth_resolution_envelope_id": truth_resolution_envelope_id,
		"truth_resolution_authority_epoch": truth_resolution_authority_epoch,
		"truth_resolution_owner_lane": truth_resolution_owner_lane,
		"truth_resolution_resolution_class": truth_resolution_resolution_class,
		"truth_resolution_visibility_class": truth_resolution_visibility_class,
		"truth_resolution_contradiction_disposition": truth_resolution_contradiction_disposition,
		"truth_resolution_source_constitution_hash": truth_resolution_source_constitution_hash,
		"truth_resolution_source_digest": truth_resolution_source_digest,
		"truth_resolution_public_summary_refs": truth_resolution_public_summary_refs,
		"truth_resolution_operator_refs": truth_resolution_operator_refs,
		"consequence_bridge": consequence_bridge.duplicate(true),
		"localized_attractor_surface": localized_attractor_surface.duplicate(true),
		"delvemind_governance_contract": delvemind_governance_contract.duplicate(true),
		"effect_validation_records": effect_validation_records.duplicate(true),
...
static func _public_safe_constitution_summary(raw: Dictionary) -> Dictionary:
	...
	return {
		...
		"market_regime_id": str(raw.get("market_regime_id", public_summary.get("market_regime_id", ""))).strip_edges(),
		"market_regime_family": str(raw.get("market_regime_family", public_summary.get("market_regime_family", ""))).strip_edges(),
		"active_regime_ids": _string_array(public_summary.get("active_regime_ids", raw.get("active_regime_ids", []))),
		"lifecycle_state_ids": _string_array(public_summary.get("lifecycle_state_ids", raw.get("lifecycle_state_ids", []))),
		"market_regime_lines": _string_array(public_summary.get("market_regime_lines", raw.get("market_regime_lines", []))),
		"lifecycle_lines": _string_array(public_summary.get("lifecycle_lines", raw.get("lifecycle_lines", []))),
