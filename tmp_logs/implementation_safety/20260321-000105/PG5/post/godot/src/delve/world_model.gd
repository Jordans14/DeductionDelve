...
static func build_model(profile: Dictionary, session_context: Dictionary) -> Dictionary:
	...
	var market_regimes := Array(Dictionary(world_memory).get("market_regimes", []))
	var lead_market_regime: Dictionary = Dictionary(market_regimes[0]) if not market_regimes.is_empty() else {}
	...
	var social_consequence_state: Dictionary = Dictionary(world_memory.get("social_consequence_state", {}))
	var encounter_apex_consequence_state: Dictionary = Dictionary(world_memory.get("encounter_apex_consequence_state", {}))
	...
	var residue_ids: Array[String] = []
	for residue_raw in Array(world_memory.get("residue_records", [])):
		var residue: Dictionary = Dictionary(residue_raw)
		var residue_id := str(residue.get("residue_id", "")).strip_edges()
		if not residue_id.is_empty() and not residue_ids.has(residue_id):
			residue_ids.append(residue_id)
	var civilization_market_regime_ids := _string_array(civilization_surface.get("market_regime_ids", []))
	var fallback_market_regime_id := civilization_market_regime_ids[0] if not civilization_market_regime_ids.is_empty() else ""
	var market_regime_id := str(
		lead_market_regime.get(
			"regime_id",
			fallback_market_regime_id
		)
	).strip_edges()
	var localized_attractor_surface := {
		...
		"surface_id": "localized_attractor_%s" % JSON.stringify({
			"constitution_hash": str(session_context.get("constitution_hash", "")).strip_edges(),
			"run_seed": int(session_context.get("run_seed", 0)),
		...
		"constitution_hash": str(session_context.get("constitution_hash", "")).strip_edges(),
		"run_seed": maxi(int(session_context.get("run_seed", 0)), 0),
		"market_regime_id": market_regime_id,
		"market_regime_family": str(lead_market_regime.get("regime_family", "")).strip_edges(),
		...
		"safe_mode_active": bool(safe_mode_state.get("enabled", activation_state.get("safe_mode_active", false))),
		"cognitive_dimensions": _string_array(field_climate.get("dominant_dimensions", [])),
		"artifact_consequence_tags": _string_array(civilization_surface.get("artifact_consequence_tags", [])),
		"social_pressure_tags": _string_array(social_consequence_state.get("public_evidence_tags", [])) + _string_array(social_consequence_state.get("blame_surface_tags", [])),
		"encounter_apex_tags": _string_array(civilization_surface.get("encounter_apex_consequence_tags", [])),
		"residue_ids": residue_ids.slice(0, 8),
		"summary_lines": _string_array(
			_string_array(civilization_surface.get("lines", []))
			+ _string_array(field_climate.get("summary_lines", []))
			+ _string_array(encounter_apex_consequence_state.get("lines", []))
		).slice(0, 4),
		"source_refs": ["world_memory", "civilization_surface", "governance_state", "theory_surface"]
	}
	return {
		...
		"localized_attractor_surface": localized_attractor_surface,
		"delvemind_governance_contract": Dictionary(experiment_state.get("governance_contract", {})).duplicate(true),
