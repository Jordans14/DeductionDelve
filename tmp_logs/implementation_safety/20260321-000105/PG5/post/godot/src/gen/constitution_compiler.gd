...
static func _build_cognitive_field_state(world_model: Dictionary, public_summary: Dictionary, theory_surface: Dictionary, run_identity: Dictionary = {}) -> Dictionary:
	...
	if not run_field_state.is_empty():
		...
		current["source_refs"] = _string_array(current.get("source_refs", ["creative_governance", "theory_surface", "public_summary"]))
		current["governance_contract"] = Dictionary(current.get("governance_contract", {
		...
		current["observation_status_domain"] = _string_array(current.get("observation_status_domain", ["public_safe_summary", "traceable_archive_only", "pressure_trace", "constitution_trace"]))
		return current
	...
	return {
		...
		"source_refs": ["creative_governance", "theory_surface", "public_summary"],
		"governance_contract": {
			"schema_name": "DelveMindGovernanceContract",
			"schema_version": 1,
			"runtime_authority": false,
...
static func _build_market_regime_state(world_model: Dictionary, policy: Dictionary, public_summary: Dictionary, generation_surface: Dictionary) -> Dictionary:
	...
	return {
		...
		"source_refs": ["economy_model", "cultural_model", "public_summary", "generation_surface"],
		"contradiction_heat": clampi(int(cultural_model.get("contradiction_heat", 0)), 0, 12),
		"safe_mode_active": bool(Dictionary(Dictionary(world_model.get("governance_state", {})).get("safe_mode_state", {})).get("enabled", false)),
		"localized_attractor_tags": _string_array(active_regime_ids + [primary_family])
	}
