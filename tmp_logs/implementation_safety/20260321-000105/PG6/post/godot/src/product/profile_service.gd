...
static func apply_run_record(profile: Dictionary, run_record: Dictionary, catalog: Dictionary = {}) -> Dictionary:
	...
	var localized_attractor_surface: Dictionary = Dictionary(diagnostics.get("localized_attractor_surface", Dictionary(world_memory.get("localized_attractor_surface", {})))).duplicate(true)
	var delvemind_governance_contract: Dictionary = Dictionary(Dictionary(next_profile.get("delvemind_experiment_state", {})).get("governance_contract", {})).duplicate(true)
	var effect_validation_records: Array = Array(diagnostics.get("effect_validation_records", Dictionary(world_memory).get("effect_validation_records", []))).duplicate(true)
...
static func build_final_consistency_persistence_truth_for_test(profile: Dictionary, catalog: Dictionary = {}) -> Dictionary:
	var current_catalog := PRODUCT_CATALOG_SCRIPT.load_catalog() if catalog.is_empty() else catalog
	var normalized_profile := normalize_profile(profile, current_catalog)
	var truth := {
		"last_run": Dictionary(normalized_profile.get("last_run", {})).duplicate(true),
		"world_memory": Dictionary(normalized_profile.get("world_memory", {})).duplicate(true),
		"home_overview_lines": build_home_overview_lines(normalized_profile, {}, current_catalog),
		"progression_preview_lines": build_progression_preview_lines(normalized_profile, current_catalog)
	}
	return {
		"truth": truth,
		"digest": _canonical_phase9_hash(truth),
		"refs": [
			str(Dictionary(truth.get("last_run", {})).get("seed", 0)),
			str(Dictionary(Dictionary(truth.get("last_run", {})).get("consequence_bridge", {})).get("bridge_id", "")).strip_edges(),
			str(Dictionary(Dictionary(truth.get("world_memory", {})).get("localized_attractor_surface", {})).get("surface_id", "")).strip_edges()
		]
	}
