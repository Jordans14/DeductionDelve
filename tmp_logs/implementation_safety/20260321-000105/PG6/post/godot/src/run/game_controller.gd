...
func build_run_guidance_packet_for_test(public_summary: Dictionary, branch_context: Dictionary = {}, role_payload: Dictionary = {}, runtime_state: Dictionary = {}) -> Dictionary:
	...
	var runtime_affordances: Dictionary = Dictionary(runtime_state.get("runtime_affordances", {})).duplicate(true)
	var role_name := str(role_payload.get("role", runtime_state.get("local_role", ""))).strip_edges()
	var role_service := ROLE_SERVICE_SCRIPT.new()
	var known_input_lines: Array[String] = [
		"Move: Traverse the route",
		"Jump: Commit gaps and dodge pressure"
	]
	var input_vocabulary_refs: Array[String] = known_input_lines.duplicate()
	...
	if bool(runtime_state.get("ziplining_now", false)):
		known_input_lines.append("Zipline: move sideways | Jump/Down to drop")
		input_vocabulary_refs.append("Zipline: move sideways | Jump/Down to drop")
		discoverable_channels.append("zipline_traversal")
		executable_actions.append_array(["grab_zipline", "drop_from_zipline"])
	elif bool(runtime_state.get("zipline_available", false)):
		known_input_lines.append("Up: Grab zipline")
		input_vocabulary_refs.append("Up: Grab zipline")
		discoverable_channels.append("zipline_traversal")
		executable_actions.append("grab_zipline")
	if carrying_artifact or not Array(runtime_state.get("reachable_artifact_ids", [])).is_empty() or bool(runtime_state.get("has_ground_artifact", false)):
		discoverable_channels.append("artifact_custody")
		executable_actions.append_array(["take_artifact", "drop_artifact"])
		input_vocabulary_refs.append("Q: Take Artifact")
		if carrying_artifact:
			input_vocabulary_refs.append("E: Drop")
		if extraction_active:
			discoverable_channels.append("extraction")
			executable_actions.append("hold_in_extraction")
	if bool(runtime_state.get("has_ground_item", false)) or not Array(runtime_state.get("reachable_item_ids", [])).is_empty():
		discoverable_channels.append("item_use")
		executable_actions.append("take_item")
		input_vocabulary_refs.append("Y: Take item")
	if not Array(runtime_affordances.get("input_vocabulary_refs", [])).is_empty():
		discoverable_channels = _dedupe_strings(discoverable_channels + _string_array(runtime_affordances.get("interaction_channels", [])))
		executable_actions = _dedupe_strings(executable_actions + _string_array(runtime_affordances.get("executable_actions", [])))
		input_vocabulary_refs = _dedupe_strings(input_vocabulary_refs + _string_array(runtime_affordances.get("input_vocabulary_refs", [])))
		known_input_lines = _dedupe_strings(known_input_lines + _string_array(runtime_affordances.get("public_safe_input_lines", [])))
	if role_service.can_forge(role_name) or bool(runtime_state.get("can_forge", false)):
		discoverable_channels.append("role_action")
		executable_actions.append("forge")
		input_vocabulary_refs.append("F: Forge")
	if role_service.can_sabotage(role_name) or bool(runtime_state.get("can_camera_jam", false)):
		discoverable_channels.append("role_action")
		executable_actions.append("camera_jam")
		input_vocabulary_refs.append("G: Camera Jam")
	if role_service.can_inspect(role_name) or bool(runtime_state.get("can_inspect_artifact", false)):
		discoverable_channels.append("inspection")
		executable_actions.append("inspect_artifact")
		input_vocabulary_refs.append("T: Inspect Artifact")
	input_vocabulary_refs.append("1/2/3: Callout")
	known_input_lines = _dedupe_strings(known_input_lines + input_vocabulary_refs)
	var meaningful_outcome_refs: Array[String] = []
	if carrying_artifact:
		meaningful_outcome_refs.append("artifact_custody")
	if extraction_active:
		meaningful_outcome_refs.append("extraction_window")
	var combo_contract_digest := str(runtime_affordances.get("combo_contract_digest", runtime_state.get("combo_contract_digest", ""))).strip_edges()
	if not combo_contract_digest.is_empty():
		meaningful_outcome_refs.append("combo_contract:%s" % combo_contract_digest)
	for domain in Array(runtime_affordances.get("meaningful_interaction_domains", [])):
		var domain_text := str(domain).strip_edges()
		if not domain_text.is_empty():
			meaningful_outcome_refs.append(domain_text)
	for ref_value in Array(runtime_state.get("meaningful_outcome_refs", [])):
		var ref_text := str(ref_value).strip_edges()
		if not ref_text.is_empty():
			meaningful_outcome_refs.append(ref_text)
	if not str(group_tension_bias).is_empty():
		meaningful_outcome_refs.append("social_pressure")
	if not str(convergence_axis).is_empty():
		meaningful_outcome_refs.append("artifact_line")
	var interaction_phase := str(runtime_state.get("interaction_phase", "start_state")).strip_edges()
	if interaction_phase.is_empty():
		interaction_phase = "start_state"
	var reachable_room_slots := Array(runtime_state.get("reachable_room_slots", [0, 1])).duplicate(true)
	var reachable_artifact_ids := Array(runtime_state.get("reachable_artifact_ids", [])).duplicate(true)
	var reachable_item_ids := Array(runtime_state.get("reachable_item_ids", [])).duplicate(true)
	var reachable_extraction_slot := int(runtime_state.get("reachable_extraction_slot", -1))
	var action_tip := ""
	...
	var interaction_failure_codes: Array[String] = []
	if discoverable_channels.is_empty():
		interaction_failure_codes.append("discoverability_missing")
	if reachable_room_slots.is_empty() and reachable_extraction_slot < 0:
		interaction_failure_codes.append("reachability_missing")
	if executable_actions.is_empty():
		interaction_failure_codes.append("executable_action_missing")
	if known_input_lines.is_empty():
		interaction_failure_codes.append("known_input_missing")
	if meaningful_outcome_refs.is_empty():
		interaction_failure_codes.append("meaningful_cashout_missing")
	var discoverable_interaction := interaction_failure_codes.find("discoverability_missing") == -1
	var reachable_interaction := interaction_failure_codes.find("reachability_missing") == -1
	var executable_interaction := interaction_failure_codes.find("executable_action_missing") == -1
	var known_input_interaction := interaction_failure_codes.find("known_input_missing") == -1
	var meaningful_interaction := interaction_failure_codes.find("meaningful_cashout_missing") == -1
	var known_input_reasserted_after_mutation := bool(runtime_state.get("known_input_reasserted_after_mutation", interaction_phase != "post_mutation_state"))
	if interaction_phase == "post_mutation_state" and not known_input_reasserted_after_mutation:
		interaction_failure_codes.append("known_input_not_reasserted")
	var post_mutation_solvability := bool(runtime_state.get("post_mutation_solvability", true))
	if interaction_phase == "post_mutation_state" and not post_mutation_solvability:
		interaction_failure_codes.append("dead_after_mutation")
	var no_dead_start_state := discoverable_interaction and reachable_interaction and executable_interaction and known_input_interaction and meaningful_interaction
	if interaction_phase == "start_state" and not no_dead_start_state:
		interaction_failure_codes.append("dead_start")
	var no_dead_after_mutation_state := true
	if interaction_phase == "post_mutation_state":
		no_dead_after_mutation_state = discoverable_interaction and reachable_interaction and executable_interaction and known_input_interaction and meaningful_interaction and post_mutation_solvability and known_input_reasserted_after_mutation
	var interaction_guarantee := {
		"schema_name": "InteractionGuarantee",
		"schema_version": 1,
		"check_id": "guidance_%s_%d" % [str(public_summary.get("constitution_hash", public_summary.get("constitution_id", "runtime"))).strip_edges(), int(runtime_state.get("run_seed", 0))],
		"constitution_hash": str(public_summary.get("constitution_hash", public_summary.get("constitution_id", ""))).strip_edges(),
		"run_seed": int(runtime_state.get("run_seed", 0)),
		"phase": interaction_phase,
		"discoverable_interaction": discoverable_interaction,
		"discoverable_channels": _dedupe_strings(discoverable_channels),
		"discoverable_surface_refs": [
			"game_controller.build_run_guidance_packet_for_test.run_kind_line",
			"game_controller.build_run_guidance_packet_for_test.focus_lines",
			"game_controller.build_run_guidance_packet_for_test.action_tip",
			"game_controller.build_run_guidance_packet_for_test.known_input_lines"
		],
		"reachable_interaction": reachable_interaction,
		"reachable_room_slots": reachable_room_slots,
		"reachable_artifact_ids": reachable_artifact_ids,
		"reachable_item_ids": reachable_item_ids,
		"reachable_extraction_slot": reachable_extraction_slot,
		"executable_interaction": executable_interaction,
		"executable_actions": _dedupe_strings(executable_actions),
		"known_input_interaction": known_input_interaction,
		"input_surface_refs": [
		...
		"input_vocabulary_refs": _dedupe_strings(input_vocabulary_refs),
		"public_safe_input_lines": _dedupe_strings(known_input_lines),
		"meaningful_interaction": meaningful_interaction,
		"meaningful_outcome_refs": _dedupe_strings(meaningful_outcome_refs),
		"post_mutation_solvability": post_mutation_solvability,
		"known_input_reasserted_after_mutation": known_input_reasserted_after_mutation,
		"no_dead_start_state": no_dead_start_state,
		"no_dead_after_mutation_state": no_dead_after_mutation_state,
		"mutation_ref_ids": Array(runtime_state.get("mutation_ref_ids", [])).duplicate(true),
		"failure_codes": _dedupe_strings(interaction_failure_codes)
	}
	return {
		...
		"known_input_lines": _dedupe_strings(known_input_lines),
		"input_vocabulary_refs": _dedupe_strings(input_vocabulary_refs),
		"interaction_guarantee": interaction_guarantee
	}
...
func _update_interaction_prompt(local_id: int) -> void:
	...
	if run_ended or not players.has(local_id):
		prompt_label.text = ""
		return
	var prompts: Array[String] = ["Move/Jump: Traverse and dodge under pressure"]
	var local_pos: Vector2 = players[local_id].global_position
	...
	if prompts.size() > prompt_limit:
		...
		trimmed.append(prompts[0])
		trimmed.append_array(prompts.slice(1, prompt_limit))
		if not trimmed.has("1/2/3: Callout"):
			trimmed[trimmed.size() - 1] = "1/2/3: Callout"
		prompts = trimmed
...
func _update_status() -> void:
	...
	var input_tokens: Array[String] = ["Move", "Jump"]
	var local_carried_id := int(NetworkManager.get_local_carried_artifact_id())
	if local_carried_id > 0:
		input_tokens.append("E")
	if not active_item.is_empty():
		input_tokens.append("U")
		input_tokens.append("[/]")
	if not input_tokens.is_empty():
		parts.append("Inputs %s" % "/".join(_dedupe_strings(input_tokens)))
	if NetworkManager.is_local_extraction_window_active():
...
func build_final_consistency_report_truth_for_test(
	event_log: Node,
	gameplay_snapshot: Dictionary = {},
	constitution_summary: Dictionary = {},
	peer_cards: Dictionary = {},
	...
	var previous_end_payload: Dictionary = end_payload.duplicate(true)
	end_payload = {
		"outcome_summary": Dictionary(outcome_summary).duplicate(true),
		"local_aftermath": Dictionary(local_aftermath).duplicate(true),
		"world_aftermath_refs": Array(world_aftermath_refs).duplicate(true)
	}
	var fact_lines := _format_timeline_grouped(event_log.get_recent_public(9999))
	fact_lines.append_array(
		build_public_fact_extensions_for_test(
			event_log,
			gameplay_snapshot,
			constitution_summary,
			peer_cards,
			local_peer_id_override,
			outcome_summary
		)
	)
	end_payload = previous_end_payload
	...
	var fact_text := "\n".join(fact_lines)
	var failing_fragments: Array[String] = []
	for fragment in forbidden_fragments:
		if fact_text.find(fragment) != -1:
...
func _build_public_fact_extensions(
	...
	var truth_resolution_envelope: Dictionary = {}
	if RunState != null:
		var runtime_constitution: Dictionary = Dictionary(RunState.expedition_constitution).duplicate(true)
		truth_resolution_envelope = Dictionary(runtime_constitution.get("truth_resolution_envelope", Dictionary(Dictionary(runtime_constitution.get("explanation_packet", {})).get("truth_resolution_envelope", {})))).duplicate(true)
	if truth_resolution_envelope.is_empty() and NetworkManager != null and NetworkManager.has_method("get_current_expedition_constitution"):
		var network_constitution: Dictionary = Dictionary(NetworkManager.get_current_expedition_constitution()).duplicate(true)
		truth_resolution_envelope = Dictionary(network_constitution.get("truth_resolution_envelope", Dictionary(Dictionary(network_constitution.get("explanation_packet", {})).get("truth_resolution_envelope", {})))).duplicate(true)
	var timeline_digest := ""
	...
	var authority_epoch := str(
		truth_resolution_envelope.get("authority_epoch", current_summary.get("truth_resolution_authority_epoch", ""))
	).strip_edges()
	var resolution_class := str(
		truth_resolution_envelope.get("resolution_class", current_summary.get("truth_resolution_resolution_class", ""))
	).strip_edges()
	var contradiction_disposition := str(
		truth_resolution_envelope.get(
			"contradiction_disposition",
			current_summary.get("truth_resolution_contradiction_disposition", "")
		)
	).strip_edges()
	if not authority_epoch.is_empty() or not resolution_class.is_empty() or not contradiction_disposition.is_empty():
		lines.append("[PROVENANCE] authority=%s resolution=%s disposition=%s" % [
			authority_epoch,
			resolution_class,
			contradiction_disposition
		])
...
func _build_forensic_bundle(
	...
	var base_truth_resolution_envelope := Dictionary(
		phase_extensions.get(
			"truth_resolution_envelope",
			Dictionary(explanation_packet.get("truth_resolution_envelope", {}))
		)
	).duplicate(true)
	var bundle_truth_resolution_envelope := GOVERNANCE_SERVICE_SCRIPT.derive_truth_resolution_envelope(
		base_truth_resolution_envelope,
		{
			"subject_id": str(replay_identity.get("replay_id", "")).strip_edges(),
			"subject_kind": "provenance",
			"authority_epoch": "post_run_derived",
			"owner_lane": "bundle",
			"resolution_class": "bundle_only_reference",
			"visibility_class": "bundle_only",
			"source_constitution_hash": constitution_hash,
			"source_digest": str(explanation_packet.get("packet_digest", "")).strip_edges(),
			"source_refs": _string_array(phase_extensions.get("provenance_source_refs", constitution_summary.get("provenance_source_refs", []))),
			"public_summary_refs": ["public_trace_classes", "public_surface_tags", "explanation_packet_digest"],
			"operator_refs": ["provenance_source_refs", "private_trace_classes", "governance_hook_set"],
			"allowed_consumers": ["bundle", "persistence", "tests"],
			"derivation_policy": "copy_only"
		}
	)
	var bundle := {
		...
		"truth_resolution_envelope": bundle_truth_resolution_envelope.duplicate(true),
...
func _build_encounter_apex_consequence_summary(local_aftermath: Dictionary, world_aftermath_refs: Array) -> Dictionary:
	...
	var world_aftermath_ref_ids: Array[String] = []
	for aftermath_ref_raw in world_aftermath_refs:
		var aftermath_ref := Dictionary(aftermath_ref_raw)
		var aftermath_id := str(aftermath_ref.get("aftermath_id", "")).strip_edges()
		if not aftermath_id.is_empty() and not world_aftermath_ref_ids.has(aftermath_id):
			world_aftermath_ref_ids.append(aftermath_id)
	summary["encounter_apex_ref"] = {
		"encounter_apex_consequence_version": int(summary.get("encounter_apex_consequence_version", 0)),
		"encounter_resolution_state": str(summary.get("encounter_resolution_state", "")).strip_edges(),
		"apex_resolution_state": str(summary.get("apex_resolution_state", "")).strip_edges(),
		"anchored_pressures": Array(summary.get("anchored_pressures", [])).duplicate(true),
		"consequence_classes": Array(summary.get("consequence_classes", [])).duplicate(true),
		"world_aftermath_ref_ids": world_aftermath_ref_ids
	}
	return summary
...
func _build_social_consequence_summary(
	...
	var private_evidence_digest := JSON.stringify(private_evidence_tags).md5_text() if not private_evidence_tags.is_empty() else ""
	return {
		...
		"social_consequence_ref": {
			...
			"relationship_pressure": relationship_pressure,
			"blame_surface_tags": blame_surface_tags.slice(0, 8),
			"private_evidence_digest": private_evidence_digest
		}
	}
...
func _verify_run_event_stream() -> Dictionary:
	...
	if RunState != null:
		if str(RunState.constitution_hash).strip_edges().is_empty():
			failures.append("constitution_hash")
		if str(Dictionary(RunState.semantic_lock_registry).get("registry_id", "")).strip_edges().is_empty():
			failures.append("semantic_lock_registry")
	var mutation_history: Array = Array(RunState.mutation_history).duplicate(true) if RunState != null else []
	for mutation_raw in mutation_history:
		var mutation: Dictionary = Dictionary(mutation_raw)
		if bool(mutation.get("known_input_reassertion_required", false)) and not bool(mutation.get("known_input_reasserted_after_mutation", false)):
			failures.append("mutation_input_reassertion")
			break
		if not bool(mutation.get("post_mutation_solvability", true)):
			failures.append("mutation_dead_end")
			break
	if not failures.is_empty():
		result["ok"] = false
		result["checks"] = int(result.get("checks", 0)) + 2
