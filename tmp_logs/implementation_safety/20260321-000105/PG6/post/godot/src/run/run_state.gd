...
var semantic_lock_registry: Dictionary = {}
var survival_state := {
	"health_points": {},
	"stability_points": {},
	"social_trust_score": {}
...
var provenance_state := {
	"provenance_contract_version": 0,
...
func set_run(seed_value: int, chain: Array, peers: Array[int], context: Dictionary = {}) -> void:
	...
	semantic_lock_registry = Dictionary(context.get("semantic_lock_registry", expedition_constitution.get("semantic_lock_registry", {}))).duplicate(true)
	survival_state = {
	...
	provenance_state = {
		"provenance_contract_version": int(Dictionary(context.get("provenance_state", {})).get("provenance_contract_version", 0)),
		"explanation_packet_digest": str(Dictionary(context.get("provenance_state", {})).get("explanation_packet_digest", constitution_summary.get("explanation_packet_digest", ""))).strip_edges(),
		"public_surface_tags": Array(Dictionary(context.get("provenance_state", {})).get("public_surface_tags", constitution_summary.get("public_surface_tags", []))).duplicate(true),
		"provenance_source_refs": Array(Dictionary(context.get("provenance_state", {})).get("provenance_source_refs", constitution_summary.get("provenance_source_refs", []))).duplicate(true)
	}
...
func set_expedition_constitution(constitution: Dictionary) -> void:
	...
	semantic_lock_registry = Dictionary(expedition_constitution.get("semantic_lock_registry", semantic_lock_registry)).duplicate(true)
...
func build_final_consistency_runtime_truth_for_test() -> Dictionary:
	return {
		"constitution_hash": constitution_hash,
		"semantic_lock_registry_id": str(Dictionary(semantic_lock_registry).get("registry_id", "")).strip_edges(),
		"constitution_summary": constitution_summary.duplicate(true),
		"truth_state": truth_state.duplicate(true),
		"provenance_state": provenance_state.duplicate(true),
		"mutation_history": Array(mutation_history).duplicate(true),
