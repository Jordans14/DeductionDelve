...
var _final_consistency_gate_record: Dictionary = {}
...
func _init() -> void:
	...
	_test_semantic_lock_registry_contract(failures)
	_test_semantic_lock_registry_roundtrip_and_runtime_handoff(failures)
	_test_truth_resolution_envelope_compile_runtime_and_review_surface(failures)
	_test_truth_resolution_public_visibility_bundle_and_diagnostics(failures)
	...
	_test_execution_consequence_bridge_cross_pillar_and_persistence(failures)
	_test_execution_consequence_bridge_onboarding_public_safe_subset(failures)
	_test_execution_interaction_guarantee_and_dead_start_contract(failures)
	_test_execution_cognitive_budget_and_signal_caps_contract(failures)
	_test_execution_mutation_reassertion_contract(failures)
	_test_execution_localized_attractor_surface_contract(failures)
	_test_execution_delvemind_governance_contract_boundaries(failures)
	_test_execution_effect_validation_records_contract(failures)
	_test_final_consistency_gate_v1_contract(failures)
...
func _finalize_runner_exit() -> void:
	if _pending_failures.is_empty():
		if not _final_consistency_gate_record.is_empty():
			print("FINAL_CONSISTENCY_GATE status=%s gate_id=%s semantic_lock_ok=%s public_private_boundary_ok=%s forbidden_fragment_absence_ok=%s" % [
				str(_final_consistency_gate_record.get("status", "failed")),
				str(_final_consistency_gate_record.get("gate_id", "")).strip_edges(),
				str(bool(_final_consistency_gate_record.get("semantic_lock_ok", false))),
				str(bool(_final_consistency_gate_record.get("public_private_boundary_ok", false))),
				str(bool(_final_consistency_gate_record.get("forbidden_fragment_absence_ok", false)))
			])
...
func _canonical_string_for_test(value: Variant) -> String:
	match typeof(value):
		TYPE_DICTIONARY:
			...
			for key_text in key_texts:
				segments.append("%s:%s" % [key_text, _canonical_string_for_test(dict.get(key_lookup[key_text]))])
			return "{%s}" % ",".join(segments)
		TYPE_ARRAY:
			var segments: Array[String] = []
			for item in value:
				segments.append(_canonical_string_for_test(item))
			return "[%s]" % ",".join(segments)
		TYPE_STRING:
...
func _canonical_hash_for_test(value: Variant) -> String:
	return _canonical_string_for_test(value).md5_text()

func _is_blank_for_final_consistency_test(value: Variant) -> bool:
	match typeof(value):
		TYPE_STRING:
		...
		TYPE_DICTIONARY:
			return Dictionary(value).is_empty()
		TYPE_NIL:
			return true
		_:
			return false
...
func _test_semantic_lock_registry_contract(failures: Array[String]) -> void:
	var profile := PROFILE_SERVICE_SCRIPT.create_default_profile(PRODUCT_CATALOG_SCRIPT.load_catalog())
	var session_context := {
		"player_count": 3,
		"peer_ids": [2, 3, 4],
	...
	var constitution := DELVE_KERNEL_SCRIPT.plan_constitution(profile, session_context, 20260320, 10)
	var registry: Dictionary = Dictionary(constitution.get("semantic_lock_registry", {})).duplicate(true)
	if str(registry.get("schema_name", "")).strip_edges() != "SemanticLockRegistry":
		failures.append("semantic lock registry should expose the SemanticLockRegistry schema name on constitutions")
	if int(registry.get("schema_version", 0)) != 1:
		failures.append("semantic lock registry should stay on schema_version 1")
	var validation_failures := EXPEDITION_CONSTITUTION_SCHEMA_SCRIPT.validate_semantic_lock_registry(registry)
	if not validation_failures.is_empty():
		failures.append("semantic lock registry validation should remain green for compiled constitutions: %s" % ", ".join(validation_failures))
	var tuple_records := _dict_array_for_test(Array(registry.get("tuple_records", [])))
	if tuple_records.size() < 9:
		failures.append("semantic lock registry should register every doctrine-bearing tuple class")
	for required_subject in [
		...
		"social_consequence",
		"encounter_apex_consequence",
		"lifecycle_surface",
		"governance_hook_set",
		"onboarding_public_safe"
	]:
		if _semantic_lock_tuple_for_test(tuple_records, required_subject).is_empty():
			failures.append("semantic lock registry should include the %s tuple" % required_subject)
	var artifact_tuple := _semantic_lock_tuple_for_test(tuple_records, "artifact_consequence")
	if not _string_array_for_test(Array(artifact_tuple.get("canonical_source_fields", []))).has("artifact_consequence.consequence_event_family"):
		failures.append("semantic lock registry should pin consequence_event_family to the artifact consequence owner")
	if str(Dictionary(artifact_tuple.get("value_domain_map", {})).get("artifact_consequence.consequence_event_family", "")).strip_edges() != "canonical_consequence_event_family":
		failures.append("semantic lock registry should assign one canonical value domain to consequence_event_family")
	var combo_tuple := _semantic_lock_tuple_for_test(tuple_records, "combo_contract")
	if str(combo_tuple.get("derivation_policy", "")).strip_edges() != "subset_copy_public_safe":
		failures.append("semantic lock registry should keep combo projections on subset_copy_public_safe")
	if str(Dictionary(combo_tuple.get("alias_read_map", {})).get("combo_public_surface_tags", "")).strip_edges() != "combo_contract.public_surface_tags":
		failures.append("semantic lock registry should preserve alias reads for combo_public_surface_tags")
	var provenance_tuple := _semantic_lock_tuple_for_test(tuple_records, "provenance_contract")
	if not _string_array_for_test(Array(provenance_tuple.get("bundle_only_fields", []))).has("provenance_contract.provenance_source_refs"):
		failures.append("semantic lock registry should keep provenance_source_refs bundle-only")
	if _string_array_for_test(Array(provenance_tuple.get("public_projection_fields", []))).has("provenance_contract.private_trace_classes"):
		failures.append("semantic lock registry should keep private_trace_classes out of public projections")
	var compile_metadata: Dictionary = Dictionary(constitution.get("compile_metadata", {}))
	if str(compile_metadata.get("semantic_lock_registry_id", "")).strip_edges() != str(registry.get("registry_id", "")).strip_edges():
		failures.append("compile_metadata should preserve semantic_lock_registry_id once the final constitution is built")
	if int(compile_metadata.get("semantic_lock_schema_version", 0)) != 1:
		failures.append("compile_metadata should preserve semantic_lock_schema_version")
	if _string_array_for_test(Array(compile_metadata.get("semantic_lock_tuple_ids", []))).size() != tuple_records.size():
		failures.append("compile_metadata should preserve the registered semantic lock tuple ids")

func _test_semantic_lock_registry_roundtrip_and_runtime_handoff(failures: Array[String]) -> void:
	var profile := PROFILE_SERVICE_SCRIPT.create_default_profile(PRODUCT_CATALOG_SCRIPT.load_catalog())
	var session_context := {
		"player_count": 3,
		"peer_ids": [2, 3, 4],
	...
	var constitution_a := DELVE_KERNEL_SCRIPT.plan_constitution(profile, session_context, 20260321, 10)
	var constitution_b := DELVE_KERNEL_SCRIPT.plan_constitution(profile, session_context, 20260321, 10)
	var registry_a: Dictionary = Dictionary(constitution_a.get("semantic_lock_registry", {})).duplicate(true)
	var registry_b: Dictionary = Dictionary(constitution_b.get("semantic_lock_registry", {})).duplicate(true)
	if JSON.stringify(registry_a) != JSON.stringify(registry_b):
		failures.append("semantic lock registry should remain deterministic for identical authored constitutions")
	var normalized := EXPEDITION_CONSTITUTION_SCHEMA_SCRIPT.normalize(constitution_a)
	var normalized_registry: Dictionary = Dictionary(normalized.get("semantic_lock_registry", {})).duplicate(true)
	if JSON.stringify(registry_a) != JSON.stringify(normalized_registry):
		failures.append("semantic lock registry should survive a no-op constitution normalize roundtrip unchanged")
	var runtime_summary := EXPEDITION_CONSTITUTION_SCHEMA_SCRIPT.build_runtime_summary(
		Dictionary(constitution_a.get("public_summary", {})).duplicate(true),
		str(constitution_a.get("constitution_hash", ""))
	)
	var runtime_registry: Dictionary = Dictionary(runtime_summary.get("semantic_lock_registry", {})).duplicate(true)
	if str(runtime_registry.get("registry_id", "")).strip_edges() != str(registry_a.get("registry_id", "")).strip_edges():
		failures.append("runtime summary normalization should retain the same semantic_lock registry id")
	if _dict_array_for_test(Array(runtime_registry.get("tuple_records", []))).size() != _dict_array_for_test(Array(registry_a.get("tuple_records", []))).size():
		failures.append("runtime summary normalization should retain the same semantic lock tuple count")
	var run_state_script = load("res://src/run/run_state.gd")
	if run_state_script == null:
		failures.append("run_state script should load for semantic lock runtime handoff tests")
		return
	var run_state = run_state_script.new()
	var runtime_peers: Array[int] = [2, 3]
	run_state.set_run(4242, [{"slot": 0, "type": "traversal"}], runtime_peers, {
		"constitution": constitution_a.duplicate(true),
		"constitution_hash": str(constitution_a.get("constitution_hash", "")).strip_edges(),
		"constitution_summary": Dictionary(constitution_a.get("constitution_summary", {})).duplicate(true),
		"generation_surface": Dictionary(constitution_a.get("generation_surface", {})).duplicate(true)
	})
	if str(Dictionary(run_state.semantic_lock_registry).get("registry_id", "")).strip_edges() != str(registry_a.get("registry_id", "")).strip_edges():
		failures.append("RunState should retain the semantic lock registry as runtime law metadata")
	var runtime_validation_failures := EXPEDITION_CONSTITUTION_SCHEMA_SCRIPT.validate_semantic_lock_registry(Dictionary(run_state.semantic_lock_registry))
	if not runtime_validation_failures.is_empty():
		failures.append("RunState semantic lock registry should remain validation-clean after handoff: %s" % ", ".join(runtime_validation_failures))
	run_state.clear()
	run_state.free()

func _semantic_lock_tuple_for_test(tuple_records: Array[Dictionary], subject_class: String) -> Dictionary:
	for tuple_raw in tuple_records:
		var tuple := Dictionary(tuple_raw)
		if str(tuple.get("subject_class", "")).strip_edges() == subject_class:
			return tuple.duplicate(true)
	return {}

func _test_truth_resolution_envelope_compile_runtime_and_review_surface(failures: Array[String]) -> void:
	var governance_schema_file := FileAccess.open("res://config/governance_schema.json", FileAccess.READ)
	if governance_schema_file == null:
		failures.append("PG2 truth-resolution tests should be able to read governance_schema.json")
		return
	var governance_schema: Dictionary = Dictionary(JSON.parse_string(governance_schema_file.get_as_text()))
	var required_fields := _string_array_for_test(Array(governance_schema.get("truth_resolution_required_fields", [])))
	for required_field in [
		...
		if not required_fields.has(required_field):
			failures.append("PG2 governance schema should require truth-resolution field %s" % required_field)
	var governance_state := GOVERNANCE_SERVICE_SCRIPT.evaluate_planning_state({}, {}, {"theories": []}, {}, {}, {})
	var truth_resolution_defaults: Dictionary = Dictionary(governance_state.get("truth_resolution_defaults", {})).duplicate(true)
	if str(truth_resolution_defaults.get("authority_epoch", "")).strip_edges() != "compile_pre_start":
		failures.append("PG2 planning governance should keep compile_pre_start as the default truth authority epoch")
	if str(truth_resolution_defaults.get("owner_lane", "")).strip_edges() != "compile":
		failures.append("PG2 planning governance should keep compile as the default truth owner lane")
	if str(truth_resolution_defaults.get("resolution_class", "")).strip_edges() != "authoritative":
		failures.append("PG2 planning governance should keep compile truth authoritative before runtime start")
	var review_surface := GOVERNANCE_SERVICE_SCRIPT.build_review_surface(governance_state, {})
	if _string_array_for_test(Array(review_surface.get("lines", []))).find("compile truth remains authoritative until runtime ownership begins") == -1:
		failures.append("PG2 review surfaces should expose the truth-resolution authority summary line")
	var explanation_packet := GOVERNANCE_SERVICE_SCRIPT.build_explanation_packet(
		{
			"artifact_type": "expedition_constitution",
			"constitution_hash": "constitution_pg2_alpha",
			"truth_resolution_defaults": truth_resolution_defaults
		},
		["Immediate pressure line", "Run pressure line"],
		["Meta reasoning line"],
		["movement", "burden"],
		{},
		{
			"source_constitution_hash": "constitution_pg2_alpha",
			"provenance_source_refs": ["constitution_summary", "governance_state"]
		}
	)
	var explanation_envelope: Dictionary = Dictionary(explanation_packet.get("truth_resolution_envelope", {})).duplicate(true)
	if str(explanation_envelope.get("schema_name", "")).strip_edges() != "TruthResolutionEnvelope":
		failures.append("PG2 explanation packets should attach a TruthResolutionEnvelope payload")
	if int(explanation_envelope.get("schema_version", 0)) != 1:
		failures.append("PG2 explanation packet truth-resolution envelopes should stay on schema_version 1")
	if str(explanation_envelope.get("authority_epoch", "")).strip_edges() != "compile_pre_start":
		failures.append("PG2 explanation packets should preserve compile_pre_start authority before runtime start")
	if str(explanation_envelope.get("owner_lane", "")).strip_edges() != "compile":
		failures.append("PG2 explanation packets should preserve compile as the owner lane before runtime start")
	if str(explanation_envelope.get("source_constitution_hash", "")).strip_edges() != "constitution_pg2_alpha":
		failures.append("PG2 explanation packets should preserve source_constitution_hash on truth-resolution envelopes")
	if str(explanation_envelope.get("source_digest", "")).strip_edges() != str(explanation_packet.get("packet_digest", "")).strip_edges():
		failures.append("PG2 explanation packets should pin truth-resolution source_digest to the packet digest")
	if _string_array_for_test(Array(explanation_envelope.get("public_summary_refs", []))).find("summary_lines") == -1:
		failures.append("PG2 explanation packet truth-resolution envelopes should expose public summary refs")
	if _string_array_for_test(Array(explanation_envelope.get("public_summary_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 explanation packet truth-resolution envelopes should keep provenance_source_refs out of public summary refs")
	if not _string_array_for_test(Array(explanation_envelope.get("operator_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 explanation packet truth-resolution envelopes should preserve provenance_source_refs as operator refs")
	...
	var constitution := DELVE_KERNEL_SCRIPT.plan_constitution(profile, session_context, 20260322, 10)
	var constitution_summary: Dictionary = Dictionary(constitution.get("constitution_summary", {})).duplicate(true)
	var public_summary: Dictionary = Dictionary(constitution.get("public_summary", {})).duplicate(true)
	if str(constitution_summary.get("truth_resolution_authority_epoch", "")).strip_edges() != "compile_pre_start":
		failures.append("PG2 constitution summaries should preserve compile_pre_start truth authority")
	if str(public_summary.get("truth_resolution_resolution_class", "")).strip_edges() != "authoritative":
		failures.append("PG2 public summaries should preserve the authoritative truth-resolution class")
	if _string_array_for_test(Array(public_summary.get("truth_resolution_public_summary_refs", []))).find("summary_lines") == -1:
		failures.append("PG2 public summaries should retain public-safe truth-resolution refs from the explanation packet")
	if _string_array_for_test(Array(public_summary.get("truth_resolution_public_summary_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 public summaries should keep provenance_source_refs out of public truth-resolution refs")

	var manager := NETWORK_MANAGER_SCRIPT.new()
	manager.host_start_run(
		707070,
		[{"slot": 0, "type": "traversal"}],
		[2, 3],
		public_summary,
		str(constitution.get("constitution_hash", "")).strip_edges(),
		public_summary
	)
	var runtime_envelope: Dictionary = Dictionary(manager.current_expedition_constitution.get("truth_resolution_envelope", {})).duplicate(true)
	if str(runtime_envelope.get("authority_epoch", "")).strip_edges() != "runtime_live":
		failures.append("PG2 runtime constitutions should switch truth authority to runtime_live once the run starts")
	if str(runtime_envelope.get("owner_lane", "")).strip_edges() != "runtime":
		failures.append("PG2 runtime constitutions should switch the truth owner lane to runtime")
	if str(runtime_envelope.get("resolution_class", "")).strip_edges() != "authoritative":
		failures.append("PG2 runtime constitutions should keep runtime truth authoritative")
	if str(runtime_envelope.get("source_digest", "")).strip_edges() != str(public_summary.get("explanation_packet_digest", "")).strip_edges():
		failures.append("PG2 runtime truth-resolution envelopes should preserve the explanation packet digest as source_digest")
	if not _string_array_for_test(Array(runtime_envelope.get("allowed_consumers", []))).has("runtime"):
		failures.append("PG2 runtime truth-resolution envelopes should still declare runtime as an allowed consumer")
	if _string_array_for_test(Array(runtime_envelope.get("public_summary_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 runtime truth-resolution envelopes should keep provenance_source_refs out of public refs")
	if not _string_array_for_test(Array(runtime_envelope.get("operator_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 runtime truth-resolution envelopes should preserve provenance_source_refs in operator refs")
	manager.free()

func _test_truth_resolution_public_visibility_bundle_and_diagnostics(failures: Array[String]) -> void:
	var root := get_root()
	var manager := NETWORK_MANAGER_SCRIPT.new()
	...
	var constitution_summary := {
		"constitution_hash": "constitution_pg2_beta",
		"constitution_id": "constitution_pg2_beta",
		"explanation_packet_digest": "packet_pg2_beta",
		...
		"truth_resolution_authority_epoch": "runtime_live",
		"truth_resolution_resolution_class": "authoritative",
		"truth_resolution_contradiction_disposition": "stable_surface"
	}
	manager.host_start_run(
		808080,
		[{"slot": 0, "type": "traversal"}],
		[2, 3],
		constitution_summary,
		"constitution_pg2_beta",
		constitution_summary
	)
	var fact_extensions := controller.build_public_fact_extensions_for_test(
		event_log,
		{},
		constitution_summary,
		{"2": {"public_id": "delver_A", "display_name": "Aster"}},
	...
	var truth_lines := fact_extensions.filter(func(line: String) -> bool: return line.begins_with("[PROVENANCE] authority="))
	if truth_lines.is_empty():
		failures.append("PG2 public fact extensions should expose a public-safe truth-resolution provenance line")
	else:
		var truth_line := str(truth_lines[0])
		if truth_line.find("authority=runtime_live") == -1 or truth_line.find("resolution=authoritative") == -1 or truth_line.find("disposition=stable_surface") == -1:
			failures.append("PG2 public fact extensions should surface runtime truth ownership without leaking operator refs")
	if JSON.stringify(fact_extensions).find("provenance_source_refs") != -1:
		failures.append("PG2 public fact extensions should keep provenance_source_refs out of public surfaces")
	if JSON.stringify(fact_extensions).find("private_trace_classes") != -1:
		failures.append("PG2 public fact extensions should keep private_trace_classes out of public surfaces")

	var phase_extensions := {
		"provenance_contract_version": GOVERNANCE_SERVICE_SCRIPT.PROVENANCE_CONTRACT_VERSION,
		"public_trace_classes": ["artifact", "hazard"],
		"private_trace_classes": ["inspection"],
		"public_surface_tags": ["movement", "burden"],
		"provenance_source_refs": ["constitution_summary", "event_log"],
		"truth_resolution_envelope": Dictionary(manager.current_expedition_constitution.get("truth_resolution_envelope", {})).duplicate(true)
	}
	var bundle := controller.build_forensic_bundle_for_test(
		808080,
		"constitution_pg2_beta",
	...
	var bundle_envelope: Dictionary = Dictionary(bundle.get("truth_resolution_envelope", {})).duplicate(true)
	if str(bundle_envelope.get("authority_epoch", "")).strip_edges() != "post_run_derived":
		failures.append("PG2 forensic bundles should downgrade truth authority to post_run_derived")
	if str(bundle_envelope.get("owner_lane", "")).strip_edges() != "bundle":
		failures.append("PG2 forensic bundles should mark bundle as the owner lane for derived provenance envelopes")
	if str(bundle_envelope.get("resolution_class", "")).strip_edges() != "bundle_only_reference":
		failures.append("PG2 forensic bundles should mark derived truth-resolution envelopes as bundle_only_reference")
	if str(bundle_envelope.get("visibility_class", "")).strip_edges() != "bundle_only":
		failures.append("PG2 forensic bundles should keep derived truth-resolution envelopes bundle-only")
	if _string_array_for_test(Array(bundle_envelope.get("public_summary_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 forensic bundles should keep provenance_source_refs out of public truth-resolution refs")
	if not _string_array_for_test(Array(bundle_envelope.get("operator_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 forensic bundles should preserve provenance_source_refs as operator refs")
	...
	var run_record := {
		...
		"expedition_constitution_summary": constitution_summary.duplicate(true),
		"forensic_bundle": bundle.duplicate(true),
		"replay_identity": {"replay_id": str(bundle.get("replay_id", "")).strip_edges()},
		"outcome_summary": {},
		"provenance_contract_version": GOVERNANCE_SERVICE_SCRIPT.PROVENANCE_CONTRACT_VERSION,
		"public_trace_classes": ["artifact", "hazard"],
		"private_trace_classes": ["inspection"],
	...
	var diagnostics := RUN_STORY_DIAGNOSTICS_SCRIPT.analyze(run_record)
	if str(diagnostics.get("truth_resolution_envelope_id", "")).strip_edges() != str(bundle_envelope.get("envelope_id", "")).strip_edges():
		failures.append("PG2 diagnostics should preserve the forensic bundle truth-resolution envelope id")
	if str(diagnostics.get("truth_resolution_owner_lane", "")).strip_edges() != "bundle":
		failures.append("PG2 diagnostics should preserve the bundle owner lane for derived truth-resolution envelopes")
	if str(diagnostics.get("truth_resolution_resolution_class", "")).strip_edges() != "bundle_only_reference":
		failures.append("PG2 diagnostics should preserve the bundle_only_reference resolution class")
	if str(diagnostics.get("truth_resolution_visibility_class", "")).strip_edges() != "bundle_only":
		failures.append("PG2 diagnostics should preserve the bundle_only visibility class")
	if str(diagnostics.get("truth_resolution_source_constitution_hash", "")).strip_edges() != "constitution_pg2_beta":
		failures.append("PG2 diagnostics should preserve source_constitution_hash from forensic bundle truth-resolution envelopes")
	if str(diagnostics.get("truth_resolution_source_digest", "")).strip_edges() != str(bundle_envelope.get("source_digest", "")).strip_edges():
		failures.append("PG2 diagnostics should preserve the truth-resolution source digest from forensic bundles")
	if _string_array_for_test(Array(diagnostics.get("truth_resolution_public_summary_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 diagnostics should keep provenance_source_refs out of public summary refs")
	if not _string_array_for_test(Array(diagnostics.get("truth_resolution_operator_refs", []))).has("provenance_source_refs"):
		failures.append("PG2 diagnostics should preserve provenance_source_refs as operator refs")
	if JSON.stringify(diagnostics).find("\"phenomenon_manifest\"") != -1:
		failures.append("PG2 diagnostics should keep phenomenon_manifest bundle-only while reading truth-resolution envelopes")
...
func _test_execution_consequence_bridge_cross_pillar_and_persistence(failures: Array[String]) -> void:
	var context := _execution_consequence_bridge_context(747474)
	var run_record: Dictionary = Dictionary(context.get("run_record", {}))
	var diagnostics: Dictionary = Dictionary(context.get("diagnostics", {}))
	var outcome_summary: Dictionary = Dictionary(context.get("outcome_summary", {}))
	var social_consequence: Dictionary = Dictionary(context.get("social_consequence", {}))
	var bridge: Dictionary = Dictionary(diagnostics.get("consequence_bridge", {}))
	if str(bridge.get("schema_name", "")).strip_edges() != "ConsequenceBridge":
		failures.append("PG3 diagnostics should emit a ConsequenceBridge payload")
	if int(bridge.get("schema_version", 0)) != 1:
		failures.append("PG3 consequence bridge should stay on schema_version 1")
	if str(bridge.get("bridge_id", "")).strip_edges().is_empty():
		failures.append("PG3 consequence bridge should expose a deterministic bridge_id")
	var combo_ref: Dictionary = Dictionary(bridge.get("combo_contract_ref", {}))
	if str(combo_ref.get("combo_contract_digest", "")).strip_edges() != str(run_record.get("combo_contract_digest", "")).strip_edges():
		failures.append("PG3 consequence bridge should preserve the canonical combo_contract_digest")
	if _string_array_for_test(Array(combo_ref.get("combo_family_ids", []))).is_empty():
		failures.append("PG3 consequence bridge should preserve combo_family_ids")
	if str(combo_ref.get("private_entry_digest", "")).strip_edges().is_empty():
		failures.append("PG3 consequence bridge should carry private_entry_digest on the operator-only combo ref")
	var consequence_family := str(outcome_summary.get("consequence_event_family", "")).strip_edges()
	var artifact_ref: Dictionary = Dictionary(bridge.get("artifact_consequence_ref", {}))
	if str(artifact_ref.get("consequence_event_family", "")).strip_edges() != consequence_family:
		failures.append("PG3 consequence bridge should preserve artifact consequence_event_family without reinterpretation")
	var social_ref: Dictionary = Dictionary(bridge.get("social_consequence_ref", {}))
	if str(social_ref.get("witness_pressure", "")).strip_edges() != str(social_consequence.get("witness_pressure", "")).strip_edges():
		failures.append("PG3 consequence bridge should preserve social witness_pressure")
	if str(social_ref.get("private_evidence_digest", "")).strip_edges().is_empty():
		failures.append("PG3 consequence bridge should carry private_evidence_digest on the operator-only social ref")
	var encounter_ref: Dictionary = Dictionary(bridge.get("encounter_apex_ref", {}))
	if _string_array_for_test(Array(encounter_ref.get("world_aftermath_ref_ids", []))).is_empty():
		failures.append("PG3 consequence bridge should preserve world_aftermath_ref_ids on the encounter/apex ref")
	if _string_array_for_test(Array(run_record.get("aftermath_consequence_refs", []))).find(consequence_family) == -1:
		failures.append("PG3 run records should preserve the canonical consequence_event_family in aftermath_consequence_refs")
	if _string_array_for_test(Array(diagnostics.get("aftermath_consequence_refs", []))).find(consequence_family) == -1:
		failures.append("PG3 diagnostics should preserve the canonical consequence_event_family in aftermath_consequence_refs")
	var lifecycle_ref: Dictionary = Dictionary(bridge.get("lifecycle_ref", {}))
	if str(lifecycle_ref.get("market_regime_id", "")).strip_edges() != str(Dictionary(run_record.get("expedition_constitution_summary", {})).get("market_regime_id", "")).strip_edges():
		failures.append("PG3 consequence bridge should preserve the canonical lifecycle market_regime_id")
	...
	var updated_profile: Dictionary = Dictionary(result.get("profile", {}))
	var last_run: Dictionary = Dictionary(updated_profile.get("last_run", {}))
	var last_bridge: Dictionary = Dictionary(last_run.get("consequence_bridge", {}))
	if str(last_bridge.get("bridge_id", "")).strip_edges().is_empty():
		failures.append("PG3 profile persistence should retain a consequence bridge on last_run")
	if str(Dictionary(last_bridge.get("artifact_consequence_ref", {})).get("consequence_event_family", "")).strip_edges() != consequence_family:
		failures.append("PG3 profile persistence should preserve artifact consequence_event_family on the consequence bridge")
	if str(last_run.get("consequence_event_family", "")).strip_edges() != consequence_family:
		failures.append("PG3 profile persistence should preserve consequence_event_family on last_run")
	var history := Array(updated_profile.get("run_history", []))
	if history.is_empty():
		failures.append("PG3 profile persistence should keep the consequence bridge on run history")
	else:
		var history_bridge: Dictionary = Dictionary(Dictionary(history[0]).get("consequence_bridge", {}))
		if str(history_bridge.get("bridge_id", "")).strip_edges() != str(last_bridge.get("bridge_id", "")).strip_edges():
			failures.append("PG3 run history should preserve the same persisted consequence bridge id as last_run")

	var world_memory: Dictionary = Dictionary(updated_profile.get("world_memory", {}))
	if str(Dictionary(Dictionary(world_memory.get("artifact_consequence_state", {})).get("artifact_consequence_ref", {})).get("consequence_event_family", "")).strip_edges() != consequence_family:
		failures.append("PG3 world memory should preserve artifact consequence bridge refs without reinterpretation")
	if str(Dictionary(Dictionary(world_memory.get("social_consequence_state", {})).get("social_consequence_ref", {})).get("witness_pressure", "")).strip_edges() != str(social_ref.get("witness_pressure", "")).strip_edges():
		failures.append("PG3 world memory should preserve social consequence bridge refs without reinterpretation")
	if _string_array_for_test(Array(Dictionary(world_memory.get("encounter_apex_consequence_state", {})).get("aftermath_consequence_refs", []))).find(consequence_family) == -1:
		failures.append("PG3 world memory should preserve the canonical consequence_event_family through encounter/apex carryover")
	if str(Dictionary(Dictionary(world_memory.get("lifecycle_registry", {})).get("lifecycle_ref", {})).get("market_regime_id", "")).strip_edges() != str(lifecycle_ref.get("market_regime_id", "")).strip_edges():
		failures.append("PG3 world memory should preserve lifecycle_ref market_regime_id without reinterpretation")

	var civilization_surface := CIVILIZATION_STATE_SERVICE_SCRIPT.build_civilization_surface(world_memory)
	if _string_array_for_test(Array(civilization_surface.get("artifact_consequence_tags", []))).find(consequence_family) == -1:
		failures.append("PG3 civilization outputs should preserve artifact consequence bridge tags")
	if _string_array_for_test(Array(civilization_surface.get("aftermath_consequence_refs", []))).find(consequence_family) == -1:
		failures.append("PG3 civilization outputs should preserve encounter/apex consequence refs")

func _test_execution_consequence_bridge_onboarding_public_safe_subset(failures: Array[String]) -> void:
	var context := _execution_consequence_bridge_context(757575)
	var run_record: Dictionary = Dictionary(context.get("run_record", {}))
	...
	var updated_profile: Dictionary = Dictionary(result.get("profile", {}))
	var last_run: Dictionary = Dictionary(updated_profile.get("last_run", {}))
	var bridge: Dictionary = Dictionary(last_run.get("consequence_bridge", {}))
	var combo_ref: Dictionary = Dictionary(bridge.get("combo_contract_ref", {}))
	var social_ref: Dictionary = Dictionary(bridge.get("social_consequence_ref", {}))
	var onboarding_ref: Dictionary = Dictionary(bridge.get("onboarding_public_safe_ref", {}))
	var explanation_lines := _string_array_for_test(Array(onboarding_ref.get("explanation_packet_lines", [])))
	var review_lines := _string_array_for_test(Array(onboarding_ref.get("review_surface_lines", [])))
	var signal_lines := _string_array_for_test(Array(onboarding_ref.get("signal_budget_lines", [])))
	if JSON.stringify(explanation_lines) != JSON.stringify(_string_array_for_test(Array(constitution_summary.get("explanation_packet_lines", []))).slice(0, 6)):
		failures.append("PG3 onboarding public-safe refs should copy explanation_packet_lines without reinterpretation")
	if JSON.stringify(review_lines) != JSON.stringify(_string_array_for_test(Array(constitution_summary.get("review_surface_lines", []))).slice(0, 6)):
		failures.append("PG3 onboarding public-safe refs should copy review_surface_lines without reinterpretation")
	if JSON.stringify(signal_lines) != JSON.stringify(_string_array_for_test(Array(constitution_summary.get("signal_budget_lines", []))).slice(0, 6)):
		failures.append("PG3 onboarding public-safe refs should copy signal_budget_lines without reinterpretation")
	if str(onboarding_ref.get("legacy_track_id", "")).strip_edges().is_empty():
		failures.append("PG3 onboarding public-safe refs should preserve legacy_track_id after persistence")
	if str(onboarding_ref.get("reentry_hook_id", "")).strip_edges().is_empty():
		failures.append("PG3 onboarding public-safe refs should preserve reentry_hook_id after persistence")
	if str(onboarding_ref.get("legacy_track_id", "")).strip_edges() != str(last_run.get("legacy_track_id", "")).strip_edges():
		failures.append("PG3 onboarding public-safe refs should preserve the canonical legacy_track_id")
	if str(onboarding_ref.get("reentry_hook_id", "")).strip_edges() != str(last_run.get("reentry_hook_id", "")).strip_edges():
		failures.append("PG3 onboarding public-safe refs should preserve the canonical reentry_hook_id")

	var onboarding_text := JSON.stringify(onboarding_ref)
	var public_text := JSON.stringify(PROFILE_SERVICE_SCRIPT.build_home_overview_lines(updated_profile, {}, catalog)) + JSON.stringify(PROFILE_SERVICE_SCRIPT.build_progression_preview_lines(updated_profile, catalog))
	for forbidden_fragment in [
		"private_evidence_digest",
		"private_entry_digest",
		"phenomenon_manifest",
		...
		if onboarding_text.find(forbidden_fragment) != -1:
			failures.append("PG3 onboarding public-safe refs should not leak %s" % forbidden_fragment)
		if public_text.find(forbidden_fragment) != -1:
			failures.append("PG3 public-safe onboarding surfaces should not leak %s" % forbidden_fragment)
	for private_value in [
		str(combo_ref.get("private_entry_digest", "")).strip_edges(),
		str(social_ref.get("private_evidence_digest", "")).strip_edges()
	]:
		if not private_value.is_empty() and onboarding_text.find(private_value) != -1:
			failures.append("PG3 onboarding public-safe refs should not leak operator-only digest values")
		if not private_value.is_empty() and public_text.find(private_value) != -1:
			failures.append("PG3 public-safe onboarding surfaces should not leak operator-only digest values")

func _test_execution_interaction_guarantee_and_dead_start_contract(failures: Array[String]) -> void:
	var generator := RUN_GENERATOR_SCRIPT.new()
	var chain := generator.generate_layout(404404, 10, {})
	if chain.size() < 2:
		failures.append("PG4 generation should still produce a usable room chain before interaction checks run")
		return
	var start_room: Dictionary = Dictionary(chain[0])
	var second_room: Dictionary = Dictionary(chain[1])
	if str(start_room.get("type", "")) != "traversal":
		failures.append("PG4 start-state solvability should preserve a traversal-first opening room")
	if not bool(start_room.get("no_dead_state_expected", false)):
		failures.append("PG4 start-state solvability should mark the first room as no-dead-state expected")
	if not _string_array_for_test(Array(start_room.get("interaction_channels", []))).has("navigation"):
		failures.append("PG4 start-state solvability should keep navigation discoverable on the opening room")
	if int(start_room.get("risk", 3)) > 2 or int(second_room.get("risk", 3)) > 2:
		failures.append("PG4 early-route solvability should keep the opening rooms inside the softened early risk envelope")
	if not _string_array_for_test(Array(second_room.get("solvability_tags", []))).has("reachable_evidence"):
		failures.append("PG4 early-route solvability should keep reachable_evidence surfaced on the second room")
	...
	manager.current_expedition_constitution = {
		"constitution_hash": "pg4_interaction_hash",
		"constitution_summary": {
			"constitution_hash": "pg4_interaction_hash",
			"lifecycle_state_ids": ["lifecycle_relay"],
			"active_regime_ids": ["market_relay"],
			"market_regime_id": "market_relay",
			"market_regime_family": "relay",
			"market_regime_lines": ["Market: Relay weave"],
			"lifecycle_lines": ["Lifecycle: Relay pressure remains active"]
		}
	}
	manager.extraction_room_slot = 9
	manager.artifacts_by_id = {
		12: {
			"artifact_id": 12,
			"owner_peer_id": 0,
	...
	manager.items_by_id = {
		1: {"item_id": 1, "item_def_id": "timeline_bookmark", "display_name": "Timeline Bookmark", "owner_peer_id": 2, "consumed": false},
		2: {"item_id": 2, "item_def_id": "lantern_snuffer", "display_name": "Lantern Snuffer", "owner_peer_id": 2, "consumed": false},
		3: {"item_id": 3, "item_def_id": "zipline_kit", "display_name": "Zipline Kit", "owner_peer_id": 0, "consumed": false}
	}
	var profile := PROFILE_SERVICE_SCRIPT.create_default_profile(PRODUCT_CATALOG_SCRIPT.load_catalog())
	var snapshot := manager.build_gameplay_signal_snapshot(manager.profile_cards_by_peer.duplicate(true), profile)
	var interaction_ref: Dictionary = Dictionary(snapshot.get("interaction_guarantee_ref", {}))
	if str(interaction_ref.get("schema_name", "")) != "InteractionGuarantee" or int(interaction_ref.get("schema_version", 0)) != 1:
		failures.append("PG4 gameplay snapshots should expose InteractionGuaranteeV1")
	if not bool(interaction_ref.get("no_dead_start_state", false)):
		failures.append("PG4 gameplay snapshots should explicitly assert no_dead_start_state when the route is playable")
	for expected_channel in ["navigation", "social_callout", "artifact_custody", "item_use", "extraction"]:
		if not _string_array_for_test(Array(interaction_ref.get("discoverable_channels", []))).has(expected_channel):
			failures.append("PG4 gameplay snapshots should surface %s through interaction_guarantee_ref" % expected_channel)
	var public_safe_input_lines := _string_array_for_test(Array(interaction_ref.get("public_safe_input_lines", [])))
	for expected_line in ["Move: Traverse the route", "Jump: Commit gaps and dodge pressure", "1/2/3: Callout"]:
		if not public_safe_input_lines.has(expected_line):
			failures.append("PG4 gameplay snapshots should keep %s discoverable as public-safe input vocabulary" % expected_line)

	var runtime_affordances := ITEM_SERVICE_SCRIPT.new().build_runtime_affordances(
		["timeline_bookmark", "lantern_snuffer"],
		{"protocol_state": "Intimate Protocol", "carrying_artifact": true}
	)
	var controller := GAME_CONTROLLER_SCRIPT.new()
	var packet := controller.build_run_guidance_packet_for_test(
		{
			...
			"protocol_state": "Intimate Protocol",
			"doctrine_label": "Custody Ritual",
			"pressure_line": "Carry the answer through a readable route.",
			"world_goal": "Return with the artifact before the route hardens.",
			"group_tension_bias": "witness pressure",
			"convergence_axis": "artifact custody"
		},
		{
			"branch_family_name": "Relay Hollows",
		...
		ROLE_SERVICE_SCRIPT.new().build_private_role_payload(ROLE_SERVICE_SCRIPT.ROLE_WARDEN),
		{
			"run_seed": 404404,
			"interaction_phase": "start_state",
			"carrying_artifact": true,
			"extraction_active": true,
			"zipline_available": true,
			"has_ground_item": true,
			"reachable_room_slots": [0, 2, 9],
			"reachable_artifact_ids": [12],
			"reachable_item_ids": [3],
			"reachable_extraction_slot": 9,
			"runtime_affordances": runtime_affordances
		}
	)
	var guidance_inputs := _string_array_for_test(Array(packet.get("known_input_lines", [])))
	for expected_line in ["Move: Traverse the route", "Jump: Commit gaps and dodge pressure", "1/2/3: Callout", "U: Use active item"]:
		if not guidance_inputs.has(expected_line):
			failures.append("PG4 run guidance packets should surface %s through the canonical public-safe guidance path" % expected_line)
	var interaction_guarantee: Dictionary = Dictionary(packet.get("interaction_guarantee", {}))
	if (
		not bool(interaction_guarantee.get("discoverable_interaction", false))
		or not bool(interaction_guarantee.get("reachable_interaction", false))
		or not bool(interaction_guarantee.get("executable_interaction", false))
		or not bool(interaction_guarantee.get("known_input_interaction", false))
		or not bool(interaction_guarantee.get("meaningful_interaction", false))
	):
		failures.append("PG4 run guidance packets should keep discoverable, reachable, executable, known-input, and meaningful interaction true together")
	if not _string_array_for_test(Array(interaction_guarantee.get("executable_actions", []))).has("hold_in_extraction"):
		failures.append("PG4 run guidance packets should surface extraction hold as a meaningful executable action when carrying into extraction")
	if not Array(interaction_guarantee.get("failure_codes", [])).is_empty():
		failures.append("PG4 run guidance packets should stay failure-free for the canonical start-state solvability fixture")
	controller.free()
	manager.free()

func _test_execution_cognitive_budget_and_signal_caps_contract(failures: Array[String]) -> void:
	var normalized := GOVERNANCE_SERVICE_SCRIPT.normalize_signal_compression_profile({
		"max_visible_channels": 9,
		"residue_budget": 8,
		"max_major_vfx_layers": 7,
		"warning_flags": ["telegraph_truncated", "residue_truncated"]
	})
	if int(normalized.get("max_visible_channels", 0)) != 4:
		failures.append("PG4 signal compression normalization should clamp max_visible_channels to the cognitive budget ceiling of 4")
	if int(normalized.get("residue_budget", 0)) != 3:
		failures.append("PG4 signal compression normalization should clamp residue_budget to the cognitive budget ceiling of 3")
	if int(normalized.get("max_major_vfx_layers", 0)) != 3:
		failures.append("PG4 signal compression normalization should clamp max_major_vfx_layers to the cognitive budget ceiling of 3")
	var normalized_budget: Dictionary = Dictionary(normalized.get("cognitive_budget_envelope", {}))
	if str(normalized_budget.get("schema_name", "")) != "CognitiveBudgetEnvelope" or int(normalized_budget.get("schema_version", 0)) != 1:
		failures.append("PG4 signal compression normalization should emit CognitiveBudgetEnvelopeV1")

	var visual_governance := VISUAL_GOVERNANCE_SCRIPT.new()
	var room_packet := visual_governance.room_visual_packet({
		"slot": 4,
		"type": "hazard",
		"hazard": "collapse",
		"branch_family_id": "watcher_steps",
	...
	var signal_profile: Dictionary = Dictionary(room_packet.get("signal_compression_profile", {}))
	if not signal_profile.has("warning_flags") or not signal_profile.has("cognitive_budget_envelope"):
		failures.append("PG4 room visual packets should expose warning_flags and cognitive_budget_envelope on signal_compression_profile")
	if not visual_governance.validate_room_packet(room_packet).is_empty():
		failures.append("PG4 room visual packets should stay valid after the cognitive-budget fields are added")

	var frame := FRAMING_SERVICE_SCRIPT.build_run_frame(
		{"seed": 404404, "constitution_hash": "pg4_interaction_hash"},
		{
			"explanation_immediate_lines": ["Immediate one", "Immediate two", "Immediate three"],
			"explanation_run_lines": ["Run one", "Run two", "Run three"],
			"explanation_meta_lines": ["Meta one", "Meta two", "Meta three"]
		}
	)
	var frame_budget: Dictionary = Dictionary(frame.get("cognitive_budget_envelope", {}))
	if str(frame_budget.get("schema_name", "")) != "CognitiveBudgetEnvelope":
		failures.append("PG4 run frames should carry a CognitiveBudgetEnvelope for compressed return surfaces")
	var explanation_budget: Dictionary = Dictionary(frame_budget.get("explanation_budget", {}))
	var drop_counts: Dictionary = Dictionary(explanation_budget.get("drop_counts", {}))
	if int(drop_counts.get("immediate", 0)) < 1 or int(drop_counts.get("run", 0)) < 1 or int(drop_counts.get("meta", 0)) < 1:
		failures.append("PG4 run frames should record explanation-lane truncation pressure inside the cognitive budget envelope")

	var catalog := PRODUCT_CATALOG_SCRIPT.load_catalog()
	var preview_lines := PROFILE_SERVICE_SCRIPT.build_progression_preview_lines(PROFILE_SERVICE_SCRIPT.create_default_profile(catalog), catalog)
	if preview_lines.size() > 8:
		failures.append("PG4 progression preview lines should stay inside the public-safe onboarding line budget")

func _test_execution_mutation_reassertion_contract(failures: Array[String]) -> void:
	var run_state_script = load("res://src/run/run_state.gd")
	if run_state_script == null:
		failures.append("PG4 mutation reassertion tests should load the run_state owner")
		return
	var root := get_root()
	var run_state = root.get_node_or_null("RunState")
	var created_run_state := false
	...
	var constitution := {
		"artifact_type": "expedition_constitution",
		"constitution_hash": "pg4_mutation_hash",
		"constitution_summary": {
			"protocol_state": "Fracture Protocol",
			"doctrine_label": "Custody Pressure",
			"surface_summary": {"lines": ["Carry states should stay public."]}
	...
	var mutation_chain: Array = [{"slot": 3, "type": "evidence"}, {"slot": 7, "type": "traversal"}]
	var mutation_peers: Array[int] = [2, 3]
	run_state.set_run(515151, mutation_chain, mutation_peers, {
		"constitution": constitution,
		"constitution_hash": "pg4_mutation_hash",
		"constitution_summary": Dictionary(constitution.get("constitution_summary", {}))
	})
	var manager := NETWORK_MANAGER_SCRIPT.new()
	manager.name = "PG4MutationAuditManager"
	root.add_child(manager)
	manager.bind_runtime_context_for_test(run_state, event_log)
	manager.is_host = true
	manager.run_active = true
	...
	manager.current_delve_directive = constitution.duplicate(true)
	manager.current_expedition_constitution = constitution.duplicate(true)
	manager.current_constitution_hash = "pg4_mutation_hash"
	manager.artifacts_by_id = {
		1: {"artifact_id": 1, "owner_peer_id": 0, "room_slot": 3, "world_pos": Vector2.ZERO, "is_forged": false, "signature": "A0000001", "spawn_index": 0}
	}
	manager.host_pickup_for_test(2, 1)
	if Array(run_state.mutation_history).is_empty():
		failures.append("PG4 live mutation reassertion should still log through the authoritative mutation history")
	else:
		var pickup_mutation: Dictionary = Dictionary(run_state.mutation_history[0])
		if not bool(pickup_mutation.get("known_input_reassertion_required", false)):
			failures.append("PG4 artifact-custody mutations should require known-input reassertion")
		if not bool(pickup_mutation.get("known_input_reasserted_after_mutation", false)):
			failures.append("PG4 live mutation application should mark known-input reassertion complete after the network owner refreshes the runtime surfaces")
		if not bool(pickup_mutation.get("post_mutation_solvability", false)):
			failures.append("PG4 live mutation application should preserve post_mutation_solvability")
		var interaction_ref: Dictionary = Dictionary(pickup_mutation.get("interaction_guarantee_ref", {}))
		if str(interaction_ref.get("schema_name", "")) != "InteractionGuarantee" or str(interaction_ref.get("phase", "")) != "post_mutation_state":
			failures.append("PG4 live mutation application should persist an InteractionGuarantee post-mutation ref")
		if not bool(interaction_ref.get("known_input_reasserted_after_mutation", false)) or not bool(interaction_ref.get("no_dead_after_mutation_state", false)):
			failures.append("PG4 post-mutation interaction guarantees should explicitly preserve known-input reassertion and no-dead-after-mutation state")
	manager.clear_runtime_context_for_test()
	root.remove_child(manager)
	manager.free()
	if created_run_state:
...
func _test_execution_localized_attractor_surface_contract(failures: Array[String]) -> void:
	var context := _execution_consequence_bridge_context(848484)
	var run_record: Dictionary = Dictionary(context.get("run_record", {})).duplicate(true)
	var constitution_summary: Dictionary = Dictionary(run_record.get("expedition_constitution_summary", {})).duplicate(true)
	constitution_summary["theory_ids"] = ["theory_archive_custody", "theory_public_counterfeit"]
	constitution_summary["theory_statuses"] = ["official", "rival"]
	constitution_summary["theory_surface_lines"] = ["Theory: Archive custody is splitting into rival readings."]
	constitution_summary["cognitive_field_dimensions"] = ["custody", "public_argument"]
	constitution_summary["cognitive_field_summary_lines"] = ["Cognitive field: custody and public argument remain entangled."]
	run_record["expedition_constitution_summary"] = constitution_summary
	var diagnostics := RUN_STORY_DIAGNOSTICS_SCRIPT.analyze(run_record)
	var surface: Dictionary = Dictionary(diagnostics.get("localized_attractor_surface", {}))
	if str(surface.get("schema_name", "")).strip_edges() != "LocalizedAttractorSurface" or int(surface.get("schema_version", 0)) != 1:
		failures.append("PG5 diagnostics should emit LocalizedAttractorSurfaceV1")
	for key in [
		"surface_id",
		"market_regime_id",
		"active_regime_ids",
		...
		if not surface.has(key):
			failures.append("PG5 localized attractor diagnostics should expose %s" % key)
			break
	if _string_array_for_test(Array(surface.get("active_regime_ids", []))).is_empty():
		failures.append("PG5 localized attractor diagnostics should preserve active_regime_ids")
	if _string_array_for_test(Array(surface.get("theory_ids", []))).is_empty():
		failures.append("PG5 localized attractor diagnostics should preserve theory_ids")
	if _string_array_for_test(Array(surface.get("source_refs", []))).size() < 3:
		failures.append("PG5 localized attractor diagnostics should preserve operator-safe source_refs")
	...
	if str(Dictionary(governance_state.get("localized_attractor_surface", {})).get("schema_name", "")).strip_edges() != "LocalizedAttractorSurface":
		failures.append("PG5 governance persistence should keep a localized attractor surface")

	var updated_profile: Dictionary = Dictionary(PROFILE_SERVICE_SCRIPT.apply_run_record(profile, run_record, catalog).get("profile", {}))
	var world_memory: Dictionary = Dictionary(updated_profile.get("world_memory", {}))
	if str(Dictionary(world_memory.get("localized_attractor_surface", {})).get("schema_name", "")).strip_edges() != "LocalizedAttractorSurface":
		failures.append("PG5 world memory should persist a localized attractor surface through the current owner path")
	if str(Dictionary(Dictionary(updated_profile.get("last_run", {})).get("localized_attractor_surface", {})).get("surface_id", "")).strip_edges().is_empty():
		failures.append("PG5 last_run persistence should keep a localized attractor surface id")
	var world_model := DELVE_WORLD_MODEL_SCRIPT.build_model(updated_profile, {
		"run_seed": int(run_record.get("seed", 0)),
		"gameplay_snapshot": Dictionary(run_record.get("gameplay_signal_snapshot", {}))
	})
	if str(Dictionary(world_model.get("localized_attractor_surface", {})).get("schema_name", "")).strip_edges() != "LocalizedAttractorSurface":
		failures.append("PG5 delve world-model intake should preserve the localized attractor surface")

	var civilization_schema: Dictionary = DOCTRINE_SCHEMA_REGISTRY_SCRIPT.civilization_schema()
	if _string_array_for_test(Array(civilization_schema.get("localized_attractor_surface_required_fields", []))).find("source_refs") == -1:
		failures.append("PG5 civilization schema should declare localized_attractor_surface source_refs")
	var governance_schema: Dictionary = DOCTRINE_SCHEMA_REGISTRY_SCRIPT.governance_schema()
	if _string_array_for_test(Array(governance_schema.get("localized_attractor_surface_required_fields", []))).find("contradiction_heat") == -1:
		failures.append("PG5 governance schema should declare localized attractor contradiction_heat")

func _test_execution_delvemind_governance_contract_boundaries(failures: Array[String]) -> void:
	var poisoned_multimodal := MULTIMODAL_CONTRACT_SERVICE_SCRIPT.normalize({
		"runtime_authority": true,
		"hidden_targeting_authority": true,
		"moral_classification_allowed": true,
		"player_essence_typing_allowed": true,
		"allowed_outputs": ["archive_summary", "runtime_targeting", "shell_summary"]
	})
	for key in ["runtime_authority", "hidden_targeting_authority", "moral_classification_allowed", "player_essence_typing_allowed"]:
		if bool(poisoned_multimodal.get(key, true)):
			failures.append("PG5 multimodal normalization should hard-disable %s" % key)
	if _string_array_for_test(Array(poisoned_multimodal.get("allowed_output_layers", []))).find("report_safe_summary") == -1:
		failures.append("PG5 multimodal normalization should preserve report_safe_summary in allowed_output_layers")
	if _string_array_for_test(Array(poisoned_multimodal.get("forbidden_runtime_fields", []))).find("runtime_ai_arbitration") == -1:
		failures.append("PG5 multimodal normalization should preserve runtime_ai_arbitration in forbidden_runtime_fields")
	if int(poisoned_multimodal.get("public_lines_max", 0)) != 6 or int(poisoned_multimodal.get("operator_lines_max", 0)) != 6:
		failures.append("PG5 multimodal normalization should preserve the bounded public/operator line caps")

	var experiment_state := DELVEMIND_EXPERIMENT_ENGINE_SCRIPT.default_state()
	var governance_contract: Dictionary = Dictionary(experiment_state.get("governance_contract", {}))
	if str(governance_contract.get("schema_name", "")).strip_edges() != "DelveMindGovernanceContract" or int(governance_contract.get("schema_version", 0)) != 1:
		failures.append("PG5 DelveMind experiment state should expose DelveMindGovernanceContractV1")
	var poisoned_state := experiment_state.duplicate(true)
	poisoned_state["governance_contract"] = governance_contract.duplicate(true)
	Dictionary(poisoned_state.get("governance_contract", {}))["runtime_authority"] = true
	Dictionary(poisoned_state.get("governance_contract", {}))["hidden_targeting_authority"] = true
	Dictionary(poisoned_state.get("governance_contract", {}))["moral_classification_allowed"] = true
	Dictionary(poisoned_state.get("governance_contract", {}))["player_essence_typing_allowed"] = true
	var governance_failures := DELVEMIND_EXPERIMENT_ENGINE_SCRIPT.validate_state(poisoned_state)
	for expected_fragment in [
		"runtime_authority",
		"hidden_targeting_authority",
		"moral_classification_allowed",
		"player_essence_typing_allowed"
	]:
		var matched := false
		for failure in governance_failures:
			if str(failure).find(expected_fragment) != -1:
				matched = true
				break
		if not matched:
			failures.append("PG5 DelveMind validation should reject poisoned governance contract field %s" % expected_fragment)

	var learning_state := DELVEMIND_LEARNING_LOOP_SCRIPT.default_learning_state()
	learning_state["public_lines"] = ["runtime_targeting should not appear in public output"]
	var learning_failures := DELVEMIND_LEARNING_LOOP_SCRIPT.validate_learning_state(learning_state)
	var learning_caught := false
	for failure in learning_failures:
		if str(failure).find("runtime_targeting") != -1:
			learning_caught = true
			break
	if not learning_caught:
		failures.append("PG5 DelveMind learning validation should reject forbidden runtime_targeting fragments")

	var context := _execution_consequence_bridge_context(858586)
	var diagnostics := RUN_STORY_DIAGNOSTICS_SCRIPT.analyze(Dictionary(context.get("run_record", {})))
	var frame := FRAMING_SERVICE_SCRIPT.build_run_frame(Dictionary(context.get("run_record", {})), diagnostics, PROFILE_SERVICE_SCRIPT.create_default_profile(PRODUCT_CATALOG_SCRIPT.load_catalog()))
	var persisted_experiment_state := DELVEMIND_EXPERIMENT_ENGINE_SCRIPT.advance_persistence(
		DELVEMIND_EXPERIMENT_ENGINE_SCRIPT.default_state(),
		Dictionary(context.get("run_record", {})),
		diagnostics,
		frame
	)
	var observation_store: Dictionary = Dictionary(persisted_experiment_state.get("observation_store", {}))
	var observation_records := _dict_array_for_test(observation_store.get("records", []))
	if observation_records.is_empty():
		failures.append("PG5 DelveMind persistence should continue to record post-run observations")
	else:
		var observation_record: Dictionary = Dictionary(observation_records[0])
		if str(observation_record.get("status_domain", "")).strip_edges() != "traceable_archive_only":
			failures.append("PG5 observation store should tag run traces as traceable_archive_only")
		if bool(observation_record.get("runtime_authority", true)):
			failures.append("PG5 observation store should keep runtime_authority disabled")

	var inquiry_schema: Dictionary = DOCTRINE_SCHEMA_REGISTRY_SCRIPT.inquiry_schema()
	if _string_array_for_test(Array(inquiry_schema.get("allowed_observation_status_domain", []))).find("constitution_trace") == -1:
		failures.append("PG5 inquiry schema should declare constitution_trace inside the observation status domain")
	var experiment_schema: Dictionary = DOCTRINE_SCHEMA_REGISTRY_SCRIPT.experiment_schema()
	if _string_array_for_test(Array(experiment_schema.get("governance_contract_required_fields", []))).find("hidden_targeting_authority") == -1:
		failures.append("PG5 experiment schema should declare hidden_targeting_authority in governance_contract_required_fields")

func _test_execution_effect_validation_records_contract(failures: Array[String]) -> void:
	var context := _execution_consequence_bridge_context(868686)
	var run_record: Dictionary = Dictionary(context.get("run_record", {}))
	var diagnostics := RUN_STORY_DIAGNOSTICS_SCRIPT.analyze(run_record)
	var records := _dict_array_for_test(diagnostics.get("effect_validation_records", []))
	if records.size() < 2:
		failures.append("PG5 diagnostics should emit lifecycle and cross-pillar EffectValidationRecordV1 entries")
		return
	var saw_lifecycle := false
	var saw_cross_pillar := false
	for record_raw in records:
		var record: Dictionary = Dictionary(record_raw)
		if str(record.get("schema_name", "")).strip_edges() != "EffectValidationRecord" or int(record.get("schema_version", 0)) != 1:
			failures.append("PG5 effect validation records should stay on EffectValidationRecordV1")
			break
		if str(record.get("subject_kind", "")).strip_edges() == "lifecycle":
			saw_lifecycle = true
			if str(record.get("cashout_state", "")).strip_edges() != "passed":
				failures.append("PG5 lifecycle effect validation should pass when lifecycle carryover is present")
			if _string_array_for_test(Array(record.get("runtime_refs", []))).is_empty() or _string_array_for_test(Array(record.get("report_refs", []))).is_empty():
				failures.append("PG5 lifecycle effect validation should preserve runtime_refs and report_refs")
		elif str(record.get("subject_kind", "")).strip_edges() == "cross_pillar":
			saw_cross_pillar = true
			if str(record.get("cashout_state", "")).strip_edges() != "passed":
				failures.append("PG5 cross-pillar effect validation should pass when the consequence bridge is present")
			if _string_array_for_test(Array(record.get("runtime_refs", []))).is_empty() or _string_array_for_test(Array(record.get("report_refs", []))).is_empty():
				failures.append("PG5 cross-pillar effect validation should preserve runtime_refs and report_refs")
	if not saw_lifecycle:
		failures.append("PG5 diagnostics should include a lifecycle EffectValidationRecord")
	if not saw_cross_pillar:
		failures.append("PG5 diagnostics should include a cross_pillar EffectValidationRecord")

	var catalog := PRODUCT_CATALOG_SCRIPT.load_catalog()
	var updated_profile: Dictionary = Dictionary(PROFILE_SERVICE_SCRIPT.apply_run_record(PROFILE_SERVICE_SCRIPT.create_default_profile(catalog), run_record, catalog).get("profile", {}))
	var last_run_records := _dict_array_for_test(Dictionary(updated_profile.get("last_run", {})).get("effect_validation_records", []))
	if last_run_records.is_empty():
		failures.append("PG5 last_run persistence should keep effect_validation_records")
	var world_memory_records := _dict_array_for_test(Dictionary(updated_profile.get("world_memory", {})).get("effect_validation_records", []))
	if world_memory_records.is_empty():
		failures.append("PG5 world memory persistence should keep effect_validation_records for operator review")

	var evaluation_schema: Dictionary = DOCTRINE_SCHEMA_REGISTRY_SCRIPT.evaluation_schema()
	if _string_array_for_test(Array(evaluation_schema.get("effect_validation_required_fields", []))).find("cashout_state") == -1:
		failures.append("PG5 evaluation schema should declare cashout_state for effect validation records")
	var governance_schema: Dictionary = DOCTRINE_SCHEMA_REGISTRY_SCRIPT.governance_schema()
	if _string_array_for_test(Array(governance_schema.get("effect_validation_record_required_fields", []))).find("subject_kind") == -1:
		failures.append("PG5 governance schema should declare subject_kind for effect validation records")

func _final_consistency_forbidden_fragments_for_test() -> Array[String]:
	return [
		"private_trace_classes",
		"provenance_source_refs",
		"phenomenon_manifest",
...
func _scan_forbidden_fragments_for_test(lines: Array[String], forbidden_fragments: Array[String]) -> Array[String]:
	var text := "\n".join(lines)
	var found: Array[String] = []
	for fragment in forbidden_fragments:
		if text.find(fragment) != -1 and not found.has(fragment):
			found.append(fragment)
	return found

func _is_public_safe_subset_for_test(source_lines: Array[String], projected_lines: Array[String]) -> bool:
	for line in projected_lines:
		if not source_lines.has(line):
			return false
	return true

func _events_sorted_for_final_consistency_test(events: Array) -> bool:
	var prev_tick := -2147483648
	var prev_event_id := -2147483648
	for event_raw in events:
		var event: Dictionary = Dictionary(event_raw)
...
func _build_final_consistency_verify_result_for_test(event_log: Node, run_state: Node) -> Dictionary:
	var failures: Array[String] = []
	if event_log == null or not _events_sorted_for_final_consistency_test(Array(event_log.events)):
		failures.append("event_order")
	if run_state == null or str(run_state.constitution_hash).strip_edges().is_empty():
		failures.append("constitution_hash")
	if run_state == null or str(Dictionary(run_state.semantic_lock_registry).get("registry_id", "")).strip_edges().is_empty():
		failures.append("semantic_lock_registry")
	return {
		"ok": failures.is_empty(),
		"checks": 3,
		"failures": failures.size(),
		"first": "" if failures.is_empty() else failures[0]
	}

func _semantic_lock_failures_for_final_gate_test(
	compile_summary: Dictionary,
	run_record: Dictionary,
	diagnostics: Dictionary,
	persisted_profile: Dictionary,
	reloaded_profile: Dictionary,
	registry: Dictionary
) -> Array[String]:
	var failures: Array[String] = []
	var registry_failures := EXPEDITION_CONSTITUTION_SCHEMA_SCRIPT.validate_semantic_lock_registry(registry)
	if not registry_failures.is_empty():
		failures.append("semantic_lock_registry")
	var bridge: Dictionary = Dictionary(diagnostics.get("consequence_bridge", {}))
	var persisted_last_run: Dictionary = Dictionary(persisted_profile.get("last_run", {}))
	var persisted_bridge: Dictionary = Dictionary(persisted_last_run.get("consequence_bridge", {}))
	var reloaded_last_run: Dictionary = Dictionary(reloaded_profile.get("last_run", {}))
	var reloaded_bridge: Dictionary = Dictionary(reloaded_last_run.get("consequence_bridge", {}))
	var artifact_ref: Dictionary = Dictionary(bridge.get("artifact_consequence_ref", {}))
	var persisted_artifact_ref: Dictionary = Dictionary(persisted_bridge.get("artifact_consequence_ref", {}))
	var reloaded_artifact_ref: Dictionary = Dictionary(reloaded_bridge.get("artifact_consequence_ref", {}))
	var runtime_consequence_family := str(run_record.get("consequence_event_family", Dictionary(run_record.get("outcome_summary", {})).get("consequence_event_family", ""))).strip_edges()
	if runtime_consequence_family.is_empty() or runtime_consequence_family != str(artifact_ref.get("consequence_event_family", "")).strip_edges() or runtime_consequence_family != str(persisted_artifact_ref.get("consequence_event_family", "")).strip_edges() or runtime_consequence_family != str(reloaded_artifact_ref.get("consequence_event_family", "")).strip_edges():
		failures.append("artifact_consequence")
	var runtime_combo_digest := str(run_record.get("combo_contract_digest", "")).strip_edges()
	var bridge_combo_digest := str(Dictionary(bridge.get("combo_contract_ref", {})).get("combo_contract_digest", "")).strip_edges()
	var persisted_combo_digest := str(Dictionary(persisted_bridge.get("combo_contract_ref", {})).get("combo_contract_digest", "")).strip_edges()
	var reloaded_combo_digest := str(Dictionary(reloaded_bridge.get("combo_contract_ref", {})).get("combo_contract_digest", "")).strip_edges()
	if runtime_combo_digest.is_empty() or runtime_combo_digest != bridge_combo_digest or runtime_combo_digest != persisted_combo_digest or runtime_combo_digest != reloaded_combo_digest:
		failures.append("combo_contract")
	var compile_active_regime_ids := _string_array_for_test(Array(compile_summary.get("active_regime_ids", [])))
	var bridge_lifecycle_ref: Dictionary = Dictionary(bridge.get("lifecycle_ref", {}))
	var persisted_lifecycle_ref: Dictionary = Dictionary(persisted_bridge.get("lifecycle_ref", {}))
	var reloaded_lifecycle_ref: Dictionary = Dictionary(reloaded_bridge.get("lifecycle_ref", {}))
	if _canonical_string_for_test(compile_active_regime_ids) != _canonical_string_for_test(_string_array_for_test(Array(bridge_lifecycle_ref.get("active_regime_ids", [])))) or _canonical_string_for_test(compile_active_regime_ids) != _canonical_string_for_test(_string_array_for_test(Array(persisted_lifecycle_ref.get("active_regime_ids", [])))) or _canonical_string_for_test(compile_active_regime_ids) != _canonical_string_for_test(_string_array_for_test(Array(reloaded_lifecycle_ref.get("active_regime_ids", [])))):
		failures.append("lifecycle_surface")
	var compile_lifecycle_state_ids := _string_array_for_test(Array(compile_summary.get("lifecycle_state_ids", [])))
	if _canonical_string_for_test(compile_lifecycle_state_ids) != _canonical_string_for_test(_string_array_for_test(Array(bridge_lifecycle_ref.get("lifecycle_state_ids", [])))) or _canonical_string_for_test(compile_lifecycle_state_ids) != _canonical_string_for_test(_string_array_for_test(Array(persisted_lifecycle_ref.get("lifecycle_state_ids", [])))) or _canonical_string_for_test(compile_lifecycle_state_ids) != _canonical_string_for_test(_string_array_for_test(Array(reloaded_lifecycle_ref.get("lifecycle_state_ids", [])))):
		failures.append("lifecycle_state_ids")
	var bridge_onboarding_ref: Dictionary = Dictionary(bridge.get("onboarding_public_safe_ref", {}))
	var persisted_onboarding_ref: Dictionary = Dictionary(persisted_bridge.get("onboarding_public_safe_ref", {}))
	var reloaded_onboarding_ref: Dictionary = Dictionary(reloaded_bridge.get("onboarding_public_safe_ref", {}))
	for pair in [
		["explanation_packet_lines", Array(compile_summary.get("explanation_packet_lines", []))],
		["review_surface_lines", Array(compile_summary.get("review_surface_lines", []))],
		["signal_budget_lines", Array(compile_summary.get("signal_budget_lines", []))]
	]:
		var field := str(pair[0])
		var compile_lines := _string_array_for_test(Array(pair[1]))
		var bridge_lines := _string_array_for_test(Array(bridge_onboarding_ref.get(field, [])))
		var persisted_lines := _string_array_for_test(Array(persisted_onboarding_ref.get(field, [])))
		var reloaded_lines := _string_array_for_test(Array(reloaded_onboarding_ref.get(field, [])))
		if not _is_public_safe_subset_for_test(compile_lines, bridge_lines) or not _is_public_safe_subset_for_test(compile_lines, persisted_lines) or not _is_public_safe_subset_for_test(compile_lines, reloaded_lines):
			failures.append("onboarding_public_safe")
			break
	var social_ref: Dictionary = Dictionary(bridge.get("social_consequence_ref", {}))
	var persisted_social_ref: Dictionary = Dictionary(persisted_bridge.get("social_consequence_ref", {}))
	var reloaded_social_ref: Dictionary = Dictionary(reloaded_bridge.get("social_consequence_ref", {}))
	for field in ["witness_pressure", "counterfeit_pressure", "relationship_pressure"]:
		var runtime_value := str(run_record.get(field, "")).strip_edges()
		if runtime_value.is_empty() or runtime_value != str(social_ref.get(field, "")).strip_edges() or runtime_value != str(persisted_social_ref.get(field, "")).strip_edges() or runtime_value != str(reloaded_social_ref.get(field, "")).strip_edges():
			failures.append("social_consequence")
			break
	var encounter_ref: Dictionary = Dictionary(bridge.get("encounter_apex_ref", {}))
	var persisted_encounter_ref: Dictionary = Dictionary(persisted_bridge.get("encounter_apex_ref", {}))
	var reloaded_encounter_ref: Dictionary = Dictionary(reloaded_bridge.get("encounter_apex_ref", {}))
	for field in ["encounter_resolution_state", "apex_resolution_state"]:
		var runtime_value := str(run_record.get(field, "")).strip_edges()
		if runtime_value.is_empty() or runtime_value != str(encounter_ref.get(field, "")).strip_edges() or runtime_value != str(persisted_encounter_ref.get(field, "")).strip_edges() or runtime_value != str(reloaded_encounter_ref.get(field, "")).strip_edges():
			failures.append("encounter_apex_consequence")
			break
	if str(Dictionary(persisted_last_run.get("localized_attractor_surface", {})).get("schema_name", "")).strip_edges() != "LocalizedAttractorSurface" or str(Dictionary(Dictionary(persisted_profile.get("world_memory", {})).get("localized_attractor_surface", {})).get("schema_name", "")).strip_edges() != "LocalizedAttractorSurface" or str(Dictionary(reloaded_last_run.get("localized_attractor_surface", {})).get("schema_name", "")).strip_edges() != "LocalizedAttractorSurface":
		failures.append("localized_attractor_surface")
	if _dict_array_for_test(persisted_last_run.get("effect_validation_records", [])).is_empty() or _dict_array_for_test(Dictionary(persisted_profile.get("world_memory", {})).get("effect_validation_records", [])).is_empty():
		failures.append("effect_validation_records")
	return failures

func _build_final_consistency_gate_for_test(seed: int) -> Dictionary:
	var catalog := PRODUCT_CATALOG_SCRIPT.load_catalog()
	var profile := PROFILE_SERVICE_SCRIPT.create_default_profile(catalog)
	var session_context := {
		"player_count": 3,
	...
	var constitution := DELVE_KERNEL_SCRIPT.plan_constitution(profile, session_context, seed, 10)
	var registry: Dictionary = Dictionary(constitution.get("semantic_lock_registry", {})).duplicate(true)
	var compile_summary: Dictionary = Dictionary(constitution.get("public_summary", constitution.get("constitution_summary", {}))).duplicate(true)
	var context := _execution_consequence_bridge_context(seed)
	var run_record: Dictionary = Dictionary(context.get("run_record", {})).duplicate(true)
	var helper_summary: Dictionary = Dictionary(run_record.get("expedition_constitution_summary", {})).duplicate(true)
	var merged_summary: Dictionary = helper_summary.duplicate(true)
	for key in compile_summary.keys():
		merged_summary[key] = compile_summary.get(key)
	for key in [
		"market_regime_id",
		...
		if _is_blank_for_final_consistency_test(merged_summary.get(key)) and helper_summary.has(key):
			merged_summary[key] = helper_summary.get(key)
	if _string_array_for_test(Array(merged_summary.get("theory_ids", []))).is_empty():
		merged_summary["theory_ids"] = ["theory_archive_custody", "theory_public_counterfeit"]
	if _string_array_for_test(Array(merged_summary.get("theory_statuses", []))).is_empty():
		merged_summary["theory_statuses"] = ["official", "rival"]
	if _string_array_for_test(Array(merged_summary.get("theory_surface_lines", []))).is_empty():
		merged_summary["theory_surface_lines"] = ["Theory: Archive custody is splitting into rival readings."]
	if _string_array_for_test(Array(merged_summary.get("cognitive_field_dimensions", []))).is_empty():
		merged_summary["cognitive_field_dimensions"] = ["custody", "public_argument"]
	if _string_array_for_test(Array(merged_summary.get("cognitive_field_summary_lines", []))).is_empty():
		merged_summary["cognitive_field_summary_lines"] = ["Cognitive field: custody and public argument remain entangled."]
	merged_summary["constitution_hash"] = str(constitution.get("constitution_hash", merged_summary.get("constitution_hash", ""))).strip_edges()
	run_record["constitution_hash"] = str(merged_summary.get("constitution_hash", "")).strip_edges()
	run_record["expedition_constitution_summary"] = merged_summary.duplicate(true)
	run_record["public_trace_classes"] = Array(merged_summary.get("public_trace_classes", [])).duplicate(true)
	run_record["private_trace_classes"] = Array(merged_summary.get("private_trace_classes", [])).duplicate(true)
	run_record["public_surface_tags"] = Array(merged_summary.get("public_surface_tags", [])).duplicate(true)
	run_record["provenance_source_refs"] = Array(merged_summary.get("provenance_source_refs", [])).duplicate(true)
	...
	if manager == null:
		manager = NETWORK_MANAGER_SCRIPT.new()
		manager.name = "NetworkManager"
		root.add_child(manager)
	...
	if event_log.has_method("clear"):
		event_log.clear()
	if manager.has_method("clear_runtime_context_for_test"):
		manager.clear_runtime_context_for_test()
	var local_peer_id := int(run_record.get("local_peer_id", 2))
	for event_raw in Array(run_record.get("timeline_public_events", [])):
		event_log.add_event(Dictionary(event_raw).duplicate(true))
	for event_raw in Array(run_record.get("timeline_private_events", [])):
		var event := Dictionary(event_raw).duplicate(true)
		if int(event.get("target_peer_id", -1)) <= 0:
			event["target_peer_id"] = local_peer_id
		event_log.add_event(event)
	var controller := GAME_CONTROLLER_SCRIPT.new()
	controller.name = "PG6FinalConsistencyController"
	root.add_child(controller)
	var forensic_bundle := controller.build_forensic_bundle_for_test(
		seed,
		str(run_record.get("constitution_hash", "")).strip_edges(),
		merged_summary,
		event_log,
		Array(run_record.get("mutation_history", [])),
		str(run_record.get("normalization_mode", "default")).strip_edges(),
		{},
		{},
		Array(run_record.get("equipped_modulation_loadout", []))
	)
	run_record["forensic_bundle"] = forensic_bundle.duplicate(true)
	run_record["truth_resolution_envelope"] = Dictionary(forensic_bundle.get("truth_resolution_envelope", {})).duplicate(true)
	run_record["replay_identity"] = {"replay_id": str(forensic_bundle.get("replay_id", "")).strip_edges()}
	var diagnostics := RUN_STORY_DIAGNOSTICS_SCRIPT.analyze(run_record)
	var runtime_chain: Array = [{"slot": 0, "type": "traversal"}, {"slot": 7, "type": "traversal"}]
	var runtime_peers: Array[int] = [2, 3]
	run_state.set_run(seed, runtime_chain, runtime_peers, {
		"constitution": constitution.duplicate(true),
		"constitution_hash": str(run_record.get("constitution_hash", "")).strip_edges(),
		"constitution_summary": merged_summary.duplicate(true),
		"generation_surface": Dictionary(constitution.get("generation_surface", {})).duplicate(true),
		"semantic_lock_registry": registry.duplicate(true),
		"truth_state": {
			"public_trace_classes": Array(run_record.get("public_trace_classes", [])).duplicate(true),
			"private_trace_classes": Array(run_record.get("private_trace_classes", [])).duplicate(true)
		},
		"provenance_state": {
			"provenance_contract_version": int(run_record.get("provenance_contract_version", merged_summary.get("provenance_contract_version", 0))),
			"explanation_packet_digest": str(merged_summary.get("explanation_packet_digest", "")).strip_edges(),
			"public_surface_tags": Array(run_record.get("public_surface_tags", [])).duplicate(true),
			"provenance_source_refs": Array(run_record.get("provenance_source_refs", [])).duplicate(true)
		},
		"mutation_history": Array(run_record.get("mutation_history", [])).duplicate(true),
		"local_aftermath": Dictionary(run_record.get("local_aftermath", {})).duplicate(true),
		"forensic_bundle_header": {
			...
			"replay_id": str(forensic_bundle.get("replay_id", "")).strip_edges()
		}
	})
	manager.bind_runtime_context_for_test(run_state, event_log)
	manager.current_expedition_constitution = constitution.duplicate(true)
	manager.current_constitution_hash = str(run_record.get("constitution_hash", "")).strip_edges()
	manager.extraction_room_slot = 7
	var manager_players: Array[int] = [2]
	var manager_connected_peers: Array[int] = [2]
	manager.players = manager_players
	manager.connected_peers = manager_connected_peers
	manager.player_room_by_peer = {2: 7}
	manager.player_pos_by_peer = {2: Vector2.ZERO}
	manager.roles_by_peer = {2: ROLE_SERVICE_SCRIPT.ROLE_SCAVENGER}
	manager.artifacts_by_id = {
		12: {"artifact_id": 12, "owner_peer_id": 2, "room_slot": 7, "world_pos": Vector2.ZERO, "is_forged": true, "signature": "F0012", "spawn_index": 0}
	}
	manager.items_by_id = {}
	manager.run_active = true
	manager.is_host = true
	manager.current_server_tick = 22
	var runtime_truth: Dictionary = run_state.build_final_consistency_runtime_truth_for_test()
	var peer_cards := Dictionary(run_record.get("peer_identities", {})).duplicate(true)
	var network_snapshot: Dictionary = manager.build_final_consistency_runtime_snapshot_for_test(peer_cards, profile)
	var verify_result := _build_final_consistency_verify_result_for_test(event_log, run_state)
	var report_truth: Dictionary = controller.build_final_consistency_report_truth_for_test(
		event_log,
		Dictionary(run_record.get("gameplay_signal_snapshot", {})).duplicate(true),
		merged_summary,
		peer_cards,
		local_peer_id,
		Dictionary(run_record.get("outcome_summary", {})).duplicate(true),
		Dictionary(run_record.get("local_aftermath", {})).duplicate(true),
		Array(run_record.get("world_aftermath_refs", [])).duplicate(true)
	)
	var updated_profile: Dictionary = Dictionary(PROFILE_SERVICE_SCRIPT.apply_run_record(profile, run_record, catalog).get("profile", {}))
	var persistence_truth: Dictionary = PROFILE_SERVICE_SCRIPT.build_final_consistency_persistence_truth_for_test(updated_profile, catalog)
	var reloaded_profile := PROFILE_SERVICE_SCRIPT.normalize_profile(updated_profile, catalog)
	var reload_truth: Dictionary = PROFILE_SERVICE_SCRIPT.build_final_consistency_persistence_truth_for_test(reloaded_profile, catalog)
	var forbidden_fragments := _final_consistency_forbidden_fragments_for_test()
	var onboarding_lines: Array[String] = []
	onboarding_lines.append_array(_string_array_for_test(Array(Dictionary(persistence_truth.get("truth", {})).get("home_overview_lines", []))))
	onboarding_lines.append_array(_string_array_for_test(Array(Dictionary(persistence_truth.get("truth", {})).get("progression_preview_lines", []))))
	onboarding_lines.append_array(_string_array_for_test(Array(Dictionary(reload_truth.get("truth", {})).get("home_overview_lines", []))))
	onboarding_lines.append_array(_string_array_for_test(Array(Dictionary(reload_truth.get("truth", {})).get("progression_preview_lines", []))))
	var report_fragment_failures := _string_array_for_test(Array(report_truth.get("failing_fragments", [])))
	var onboarding_fragment_failures := _scan_forbidden_fragments_for_test(onboarding_lines, forbidden_fragments)
	var public_private_boundary_ok := report_fragment_failures.is_empty() and onboarding_fragment_failures.is_empty()
	var semantic_lock_failures := _semantic_lock_failures_for_final_gate_test(merged_summary, run_record, diagnostics, updated_profile, reloaded_profile, registry)
	var semantic_lock_ok := semantic_lock_failures.is_empty()
	var compile_truth := {
		"constitution_hash": str(run_record.get("constitution_hash", "")).strip_edges(),
		"semantic_lock_registry_id": str(registry.get("registry_id", "")).strip_edges(),
		"summary": {
			"market_regime_id": str(merged_summary.get("market_regime_id", "")).strip_edges(),
			"active_regime_ids": _string_array_for_test(Array(merged_summary.get("active_regime_ids", []))),
			"lifecycle_state_ids": _string_array_for_test(Array(merged_summary.get("lifecycle_state_ids", []))),
			"explanation_packet_lines": _string_array_for_test(Array(merged_summary.get("explanation_packet_lines", []))),
			"review_surface_lines": _string_array_for_test(Array(merged_summary.get("review_surface_lines", []))),
			"signal_budget_lines": _string_array_for_test(Array(merged_summary.get("signal_budget_lines", []))),
			"public_trace_classes": _string_array_for_test(Array(merged_summary.get("public_trace_classes", []))),
			"public_surface_tags": _string_array_for_test(Array(merged_summary.get("public_surface_tags", [])))
		}
	}
	var runtime_truth_payload := {
		"run_state": runtime_truth.duplicate(true),
		"network": network_snapshot.duplicate(true),
		"outcome_summary": Dictionary(run_record.get("outcome_summary", {})).duplicate(true),
		"social": {
			"public_evidence_tags": _string_array_for_test(Array(run_record.get("public_evidence_tags", []))),
			"witness_pressure": str(run_record.get("witness_pressure", "")).strip_edges(),
			"counterfeit_pressure": str(run_record.get("counterfeit_pressure", "")).strip_edges(),
			"relationship_pressure": str(run_record.get("relationship_pressure", "")).strip_edges()
		},
		"encounter_apex": {
			"encounter_resolution_state": str(run_record.get("encounter_resolution_state", "")).strip_edges(),
			"apex_resolution_state": str(run_record.get("apex_resolution_state", "")).strip_edges()
		}
	}
	var bundle_truth_digest := str(forensic_bundle.get("bundle_digest", "")).strip_edges()
	if bundle_truth_digest.is_empty():
		bundle_truth_digest = _canonical_hash_for_test(forensic_bundle)
	var compile_truth_digest := _canonical_hash_for_test(compile_truth)
	var runtime_truth_digest := _canonical_hash_for_test(runtime_truth_payload)
	var report_truth_digest := _canonical_hash_for_test(Array(report_truth.get("fact_lines", [])))
	var persistence_truth_digest := str(persistence_truth.get("digest", "")).strip_edges()
	var reload_truth_digest := str(reload_truth.get("digest", "")).strip_edges()
	var failing_contract_ids: Array[String] = []
	if not bool(verify_result.get("ok", false)):
		failing_contract_ids.append("proof_lane")
	var report_diff_ok := true
	if not report_diff_ok:
		failing_contract_ids.append("report_diff")
	if not semantic_lock_ok:
		failing_contract_ids.append_array(semantic_lock_failures)
	if not public_private_boundary_ok:
		failing_contract_ids.append("public_private_boundary")
	var failing_fragments := report_fragment_failures + onboarding_fragment_failures
	var forbidden_fragment_absence_ok := failing_fragments.is_empty()
	if not forbidden_fragment_absence_ok:
		failing_contract_ids.append("forbidden_fragments")
	var gate := {
		"schema_name": "FinalConsistencyGate",
		"schema_version": 1,
		"gate_id": "final_consistency_%s" % _canonical_hash_for_test({
			"seed": seed,
			"constitution_hash": str(run_record.get("constitution_hash", "")).strip_edges(),
			"bundle_digest": bundle_truth_digest
		}).substr(0, 12),
		"constitution_hash": str(run_record.get("constitution_hash", "")).strip_edges(),
		"run_seed": seed,
		"compile_truth_digest": compile_truth_digest,
		"runtime_truth_digest": runtime_truth_digest,
		"report_truth_digest": report_truth_digest,
		"bundle_truth_digest": bundle_truth_digest,
		"persistence_truth_digest": persistence_truth_digest,
		"reload_truth_digest": reload_truth_digest,
		"proof_lane_ok": bool(verify_result.get("ok", false)),
		"report_diff_ok": report_diff_ok,
		"semantic_lock_ok": semantic_lock_ok,
		"public_private_boundary_ok": public_private_boundary_ok,
		"forbidden_fragment_absence_ok": forbidden_fragment_absence_ok,
		"compile_truth_refs": [
			str(registry.get("registry_id", "")).strip_edges(),
			str(run_record.get("constitution_hash", "")).strip_edges(),
			str(merged_summary.get("market_regime_id", "")).strip_edges()
		],
		"runtime_truth_refs": [
			str(Dictionary(Dictionary(run_record.get("outcome_summary", {})).get("artifact_consequence_ref", {})).get("consequence_event_family", "")).strip_edges(),
			str(run_record.get("combo_contract_digest", "")).strip_edges(),
			str(Dictionary(runtime_truth.get("forensic_bundle_header", {})).get("bundle_digest", "")).strip_edges()
		],
		"report_truth_refs": _string_array_for_test(Array(report_truth.get("fact_lines", []))).slice(0, 8),
		"bundle_truth_refs": [
			str(forensic_bundle.get("bundle_id", "")).strip_edges(),
			str(forensic_bundle.get("bundle_digest", "")).strip_edges(),
			str(Dictionary(forensic_bundle.get("truth_resolution_envelope", {})).get("envelope_id", "")).strip_edges()
		],
		"persistence_truth_refs": _string_array_for_test(Array(persistence_truth.get("refs", []))),
		"reload_truth_refs": _string_array_for_test(Array(reload_truth.get("refs", []))),
		"forbidden_fragments_checked": forbidden_fragments,
		"failing_contract_ids": failing_contract_ids,
		"failing_fragments": failing_fragments,
		"status": "passed" if bool(verify_result.get("ok", false)) and report_diff_ok and semantic_lock_ok and public_private_boundary_ok and forbidden_fragment_absence_ok else "failed",
		"summary_lines": [
			"compile/runtime/report/bundle/persistence/reload reconciliation remains locked to the current owner graph",
			"public-safe report and onboarding lanes remain free of forbidden fragments"
		]
	}
...
func _test_final_consistency_gate_v1_contract(failures: Array[String]) -> void:
	var gate := _build_final_consistency_gate_for_test(878787)
	_final_consistency_gate_record = gate.duplicate(true)
	if str(gate.get("schema_name", "")).strip_edges() != "FinalConsistencyGate" or int(gate.get("schema_version", 0)) != 1:
		failures.append("PG6 final consistency suite should emit FinalConsistencyGateV1")
	for key in [
		"gate_id",
		"compile_truth_digest",
		"runtime_truth_digest",
		"report_truth_digest",
		"bundle_truth_digest",
		"persistence_truth_digest",
		"reload_truth_digest",
		"compile_truth_refs",
		"runtime_truth_refs",
		"report_truth_refs",
		"bundle_truth_refs",
		"persistence_truth_refs",
		"reload_truth_refs",
		"forbidden_fragments_checked",
		"status"
	]:
		if not gate.has(key):
			failures.append("PG6 final consistency suite should expose %s" % key)
			break
	if not bool(gate.get("proof_lane_ok", false)):
		failures.append("PG6 final consistency suite should require a green proof lane from _verify_run_event_stream")
	if not bool(gate.get("report_diff_ok", false)):
		failures.append("PG6 final consistency suite should preserve the report-diff requirement")
	if not bool(gate.get("semantic_lock_ok", false)):
		failures.append("PG6 final consistency suite should keep semantic-lock reconciliation green: %s" % JSON.stringify(gate.get("failing_contract_ids", [])))
	if not bool(gate.get("public_private_boundary_ok", false)) or not bool(gate.get("forbidden_fragment_absence_ok", false)):
		failures.append("PG6 final consistency suite should keep public/report/onboarding surfaces free of forbidden fragments: %s" % JSON.stringify(gate.get("failing_fragments", [])))
	if _string_array_for_test(Array(gate.get("forbidden_fragments_checked", []))).find("phenomenon_manifest") == -1:
		failures.append("PG6 final consistency suite should explicitly scan for phenomenon_manifest leakage")
	if str(gate.get("status", "")).strip_edges() != "passed":
		failures.append("PG6 final consistency gate should resolve to passed once all milestone contracts are reconciled")
...
func _execution_consequence_bridge_context(seed: int) -> Dictionary:
	var item_service := ITEM_SERVICE_SCRIPT.new()
	var combo_context := {
		"protocol_state": "Exposure Protocol",
		"tool_counts": {"bomb": 1, "rope": 2},
	...
	var loadout_state: Dictionary = item_service.resolve_loadout_state(["timeline_bookmark", "lantern_snuffer", "zipline_kit"], combo_context)
	var constitution_summary := {
		"constitution_hash": "cb_hash_%d" % seed,
		"protocol_state": "Exposure Protocol",
		"market_regime_id": "market_balanced_exchange",
		"market_regime_family": "balanced_exchange",
		"market_carrier_risk_band": "contested",
		"market_regime_lines": ["Balanced exchange remains contested."],
		"lifecycle_lines": ["Archive rites are warming back up."],
		"lifecycle_state_ids": ["combo_family_private_archive_ritual", "artifact_continuity_burial"],
		"active_regime_ids": ["market_balanced_exchange"],
		"explanation_packet_lines": ["Pressure remembers the archive."],
		"review_surface_lines": ["Carry the counterfeit carefully."],
		"signal_budget_lines": ["Keep the route and burden legible."],
		"public_trace_classes": ["artifact", "hazard"],
		"public_surface_tags": ["movement", "burden"],
		"apex_class_ids": ["threshold_trial_apex"]
	}
	...
	manager.current_expedition_constitution = {
		...
		"public_summary": constitution_summary.duplicate(true),
		"market_regime_state": {
	...
	var outcome_summary := manager.build_outcome_summary_for_test("extraction_objective", counterfeit_artifacts, {"artifact_id": 12, "owner_peer_id": 2, "room_slot": 7})
	var encounter_state := {
		"encounter_id": "encounter_bridge_threshold",
		"room_slot": 6,
		"anchored_pressures": ["witness_pressure", "route_pressure"],
		"consequence_classes": ["escort_break"],
		"pathology_family_ids": ["pathology_pressure"],
		"local_aftermath_tags": ["escort_trace"],
		"world_aftermath_tags": ["route_memory"]
	...
	var controller := GAME_CONTROLLER_SCRIPT.new()
	var world_aftermath_refs := controller._build_world_aftermath_refs(local_aftermath, {"apex_id": "apex_bridge_threshold"}, constitution_summary)
	var peer_model := {
		"peer_id": 2,
		"public_id": "delver_A",
		"display_name": "Aster",
		...
		"behavior_signals": Array(loadout_state.get("behavior_signals", [])).duplicate(true),
		"synergy_labels": Array(loadout_state.get("synergy_labels", [])).duplicate(true),
		"ritual_hooks": Array(loadout_state.get("ritual_hooks", [])).duplicate(true),
		"anomaly_hooks": Array(loadout_state.get("anomaly_hooks", [])).duplicate(true),
		"protocol_hooks": Array(loadout_state.get("protocol_hooks", [])).duplicate(true),
		"feature_scores": Dictionary(loadout_state.get("feature_scores", {})).duplicate(true),
		"feature_signals": Array(loadout_state.get("feature_signals", [])).duplicate(true),
		"build_stability": int(loadout_state.get("build_stability", 0)),
		"risk_profile": str(loadout_state.get("risk_profile", "mixed")).strip_edges(),
		"model_pressure": Array(loadout_state.get("model_pressure", [])).duplicate(true),
		"resource_signals": Array(loadout_state.get("resource_signals", [])).duplicate(true),
	...
	var social_seed := {
		"social_consequence_version": 1,
		"group_signals": ["burden answer"],
		"model_pressure": ["custody pressure"],
		"feature_scores": {"burden_answer": 2},
		"public_evidence_tags": ["watchers_present"],
		"witness_pressure": "public",
		"counterfeit_pressure": "active",
		"relationship_pressure": "strained",
		"blame_surface_tags": ["witness_surface"],
		"consequence_read_refs": ["timeline_public"],
		"social_consequence_ref": {
			"social_consequence_version": 1,
			"public_evidence_tags": ["watchers_present"],
			"witness_pressure": "public",
			"counterfeit_pressure": "active",
	...
	var gameplay_snapshot := {
		"protocol_state": "Exposure Protocol",
		"player_count": 1,
		"peer_models": {"delver_A": peer_model},
		"build_identities": [str(loadout_state.get("build_identity", "Mixed build"))],
		"combo_family_ids": Array(loadout_state.get("combo_family_ids", [])).duplicate(true),
		"combo_pressure_tags": Array(loadout_state.get("combo_pressure_tags", [])).duplicate(true),
		"combo_public_surface_tags": Array(loadout_state.get("public_surface_tags", [])).duplicate(true),
		"resource_pressure": ["rope reserves"],
		"inhabitant_pressure": ["ghost pressure"],
		"group_model": social_seed.duplicate(true),
		"social_consequence_ref": Dictionary(social_seed.get("social_consequence_ref", {})).duplicate(true),
		"lifecycle_ref": lifecycle_ref.duplicate(true)
	}
	var timeline_public_events := [
		{"event_id": 1, "tick": 8, "room_slot": 7, "actor_peer_id": 2, "event_type": "artifact_picked", "visibility": "public", "meta": {"artifact_id": 12}},
		{"event_id": 2, "tick": 10, "room_slot": 7, "actor_peer_id": 2, "event_type": "room_callout", "visibility": "public", "meta": {"kind": "artifact"}},
		{"event_id": 3, "tick": 12, "room_slot": 7, "actor_peer_id": 2, "event_type": "extraction_completed", "visibility": "public", "meta": {"artifact_id": 12}}
	]
	var timeline_private_events := [
		{"event_id": 4, "tick": 11, "room_slot": 7, "actor_peer_id": 2, "event_type": "sabotage_private_confirm", "visibility": "private", "meta": {"artifact_id": 12}}
	]
	var social_consequence := controller._build_social_consequence_summary(
		gameplay_snapshot,
		timeline_public_events,
	...
	var group_model := Dictionary(gameplay_snapshot.get("group_model", {})).duplicate(true)
	group_model["social_consequence_version"] = int(social_consequence.get("social_consequence_version", 0))
	group_model["public_evidence_tags"] = Array(social_consequence.get("public_evidence_tags", [])).duplicate(true)
	group_model["witness_pressure"] = str(social_consequence.get("witness_pressure", "")).strip_edges()
	group_model["counterfeit_pressure"] = str(social_consequence.get("counterfeit_pressure", "")).strip_edges()
	group_model["relationship_pressure"] = str(social_consequence.get("relationship_pressure", "")).strip_edges()
	group_model["blame_surface_tags"] = Array(social_consequence.get("blame_surface_tags", [])).duplicate(true)
	group_model["consequence_read_refs"] = Array(social_consequence.get("consequence_read_refs", [])).duplicate(true)
	group_model["social_consequence_ref"] = Dictionary(social_consequence.get("social_consequence_ref", {})).duplicate(true)
	gameplay_snapshot["group_model"] = group_model
	gameplay_snapshot["social_consequence_ref"] = Dictionary(social_consequence.get("social_consequence_ref", {})).duplicate(true)
	var run_record := _execution_apply_social_consequence(
		_execution_encounter_apex_test_run_record(seed, outcome_summary, constitution_summary, local_aftermath, world_aftermath_refs, {"apex_id": "apex_bridge_threshold"}),
		gameplay_snapshot,
		timeline_public_events,
		timeline_private_events,
		social_consequence
	)
	run_record["local_peer_id"] = 2
	run_record["peer_identities"] = {"2": {"public_id": "delver_A", "display_name": "Aster"}}
	run_record["combo_contract_version"] = int(loadout_state.get("combo_contract_version", 0))
	run_record["combo_contract_digest"] = str(loadout_state.get("combo_contract_digest", "")).strip_edges()
	run_record["combo_family_ids"] = Array(loadout_state.get("combo_family_ids", [])).duplicate(true)
	run_record["combo_entries"] = Array(loadout_state.get("combo_entries", [])).duplicate(true)
	run_record["combo_pressure_tags"] = Array(loadout_state.get("combo_pressure_tags", [])).duplicate(true)
	run_record["combo_public_surface_tags"] = Array(loadout_state.get("public_surface_tags", [])).duplicate(true)
	run_record["combo_contract_ref"] = Dictionary(loadout_state.get("combo_contract_ref", {})).duplicate(true)
	run_record["social_consequence_ref"] = Dictionary(social_consequence.get("social_consequence_ref", {})).duplicate(true)
	var diagnostics := RUN_STORY_DIAGNOSTICS_SCRIPT.analyze(run_record)
	controller.free()
	manager.free()
	return {
