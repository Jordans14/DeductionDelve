...
func get_local_active_items() -> Array[Dictionary]:
	var mp := _mp()
	var local_id := _safe_mp_unique_id(mp)
	if local_id <= 0:
		return []
	return get_active_items_for_peer(local_id)
...
func get_local_loadout_runtime_affordances() -> Dictionary:
	...
	if local_id <= 0:
		return {}
	return get_loadout_runtime_affordances_for_peer(local_id)
...
func build_final_consistency_runtime_snapshot_for_test(peer_identities: Dictionary = {}, profile: Dictionary = {}) -> Dictionary:
	var constitution: Dictionary = get_current_expedition_constitution()
	var constitution_summary: Dictionary = get_current_expedition_constitution_summary()
	var gameplay_snapshot: Dictionary = build_gameplay_signal_snapshot(peer_identities, profile)
	var combo_contract_digests: Array[String] = []
	for peer_model_raw in Dictionary(gameplay_snapshot.get("peer_models", {})).values():
		...
		var combo_digest := str(peer_model.get("combo_contract_digest", "")).strip_edges()
		if not combo_digest.is_empty() and not combo_contract_digests.has(combo_digest):
			combo_contract_digests.append(combo_digest)
	return {
		"constitution_hash": str(constitution.get("constitution_hash", current_constitution_hash)).strip_edges(),
		"semantic_lock_registry_id": str(Dictionary(constitution.get("semantic_lock_registry", {})).get("registry_id", "")).strip_edges(),
		"gameplay_signal_snapshot": gameplay_snapshot.duplicate(true),
		"combo_contract_digests": combo_contract_digests,
		"lifecycle_ref": Dictionary(gameplay_snapshot.get("lifecycle_ref", {})).duplicate(true),
		"public_trace_classes": _string_array(constitution_summary.get("public_trace_classes", [])),
		"public_surface_tags": _string_array(constitution_summary.get("public_surface_tags", [])),
		"truth_resolution_envelope": Dictionary(
			constitution.get(
				"truth_resolution_envelope",
				Dictionary(Dictionary(constitution.get("explanation_packet", {})).get("truth_resolution_envelope", {}))
			)
		).duplicate(true)
	}
