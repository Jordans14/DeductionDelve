...
static func apply_mutation_plan(run_state: Node, event_log: Node, plan: Dictionary, tick: int = -1) -> Dictionary:
	...
	if event.is_empty():
		return {}
	event["tick"] = tick
	var known_input_reassertion_required := _mutation_requires_input_reassertion(event)
	event["interaction_phase"] = "post_mutation_state"
	event["known_input_reassertion_required"] = known_input_reassertion_required
	event["known_input_reasserted_after_mutation"] = not known_input_reassertion_required
	event["post_mutation_solvability"] = true
	event["interaction_guarantee_ref"] = {
		"schema_name": "InteractionGuarantee",
		"schema_version": 1,
		"check_id": "mutation_%s" % str(event.get("mutation_id", "")).strip_edges(),
		"constitution_hash": str(event.get("constitution_hash", "")).strip_edges(),
		"run_seed": int(run_state.run_seed if run_state != null else 0),
		"phase": "post_mutation_state",
		"discoverable_interaction": true,
		"discoverable_channels": ["navigation", "social_callout"],
		...
		"reachable_interaction": true,
		"reachable_room_slots": [],
		"reachable_artifact_ids": [],
		"reachable_item_ids": [],
		"reachable_extraction_slot": -1,
		"executable_interaction": true,
		"executable_actions": ["move", "jump", "callout_danger", "callout_regroup", "callout_artifact"],
		...
		"input_vocabulary_refs": ["Move: Traverse the route", "Jump: Commit gaps and dodge pressure", "1/2/3: Callout"],
		"public_safe_input_lines": ["Move: Traverse the route", "Jump: Commit gaps and dodge pressure", "1/2/3: Callout"],
		"meaningful_interaction": true,
		"meaningful_outcome_refs": ["mutation:%s" % str(event.get("mutation_id", "")).strip_edges()],
		"post_mutation_solvability": true,
		"known_input_reasserted_after_mutation": not known_input_reassertion_required,
		"no_dead_start_state": true,
		"no_dead_after_mutation_state": true,
		"mutation_ref_ids": [str(event.get("mutation_id", "")).strip_edges()],
		"failure_codes": []
	}
...
static func _mutation_requires_input_reassertion(event: Dictionary) -> bool:
	var domain := str(event.get("domain", "")).strip_edges()
	var trigger_type := str(event.get("trigger_type", "")).strip_edges()
	if domain in ["artifact_custody", "route_state", "social_trust", "readability_flags", "pressure_ecology"]:
		return true
	return trigger_type in ["artifact_picked", "artifact_dropped", "artifact_stolen", "extraction_window_started", "covenant_activated", "transformation_threshold_crossed"]
