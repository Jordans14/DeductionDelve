...
static func build_run_frame(run_record: Dictionary, diagnostics: Dictionary, profile: Dictionary = {}) -> Dictionary:
	...
	var explanation_meta_lines := _string_array(diagnostics.get("explanation_meta_lines", []))
	var explanation_drop_counts := {
		"immediate": maxi(explanation_immediate_lines.size() - 2, 0),
		"run": maxi(explanation_run_lines.size() - 2, 0),
		"meta": maxi(explanation_meta_lines.size() - 2, 0),
		"summary": 0,
		"operator": 0
	}
	var warning_flags: Array[String] = []
	for lane_id in explanation_drop_counts.keys():
		if int(explanation_drop_counts.get(lane_id, 0)) > 0:
			warning_flags.append("%s_truncated" % str(lane_id))
	var cognitive_budget_envelope := {
		"schema_name": "CognitiveBudgetEnvelope",
		"schema_version": 1,
		"budget_id": "frame_%s" % str(run_record.get("seed", 0)),
		...
		"explanation_budget": {
			...
			"drop_counts": explanation_drop_counts
		},
		"visual_budget": {
			"max_visible_channels": 4,
			"residue_budget": 3,
	...
	return {
		...
		"cognitive_budget_envelope": cognitive_budget_envelope,
		"layer_safe": true
	}
