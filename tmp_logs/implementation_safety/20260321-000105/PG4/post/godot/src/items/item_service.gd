...
func resolve_loadout_state(item_def_ids: Array[String], context: Dictionary = {}) -> Dictionary:
	...
	var state: Dictionary = synergy_service.resolve(item_def_ids, gameplay_profiles, context)
	var combo_entries: Array = Array(state.get("combo_entries", [])).duplicate(true)
	state["combo_contract_ref"] = {
		"combo_contract_version": int(state.get("combo_contract_version", 0)),
		"combo_contract_digest": str(state.get("combo_contract_digest", "")).strip_edges(),
		"combo_family_ids": Array(state.get("combo_family_ids", [])).duplicate(true),
		"combo_pressure_tags": Array(state.get("combo_pressure_tags", [])).duplicate(true),
		"public_surface_tags": Array(state.get("public_surface_tags", [])).duplicate(true),
...
func build_runtime_affordances(item_def_ids: Array[String], context: Dictionary = {}) -> Dictionary:
	...
	var category_counts := {}
	var interaction_channels: Array[String] = ["navigation", "social_callout"]
	...
	var meaningful_interaction_domains: Array[String] = ["route_pressure", "social_pressure"]
	var has_active_use_item := false
	for item_def_id in item_def_ids:
		var category_id := get_category(item_def_id)
		category_counts[category_id] = int(category_counts.get(category_id, 0)) + 1
		if is_active_use_item(item_def_id):
			has_active_use_item = true
	...
	affordances["combo_contract_ref"] = Dictionary(state.get("combo_contract_ref", {})).duplicate(true)
	...
	if has_active_use_item:
		interaction_channels.append("item_use")
		...
		meaningful_interaction_domains.append("loadout_expression")
	if not Array(affordances.get("active_covenant_ids", [])).is_empty() or not Array(affordances.get("active_transformation_ids", [])).is_empty():
		if not interaction_channels.has("role_action"):
			interaction_channels.append("role_action")
		if not meaningful_interaction_domains.has("mutation_pressure"):
			meaningful_interaction_domains.append("mutation_pressure")
	affordances["interaction_channels"] = _dedupe_strings(interaction_channels)
	affordances["executable_actions"] = _dedupe_strings(executable_actions)
	affordances["input_vocabulary_refs"] = _dedupe_strings(input_vocabulary_refs)
	affordances["public_safe_input_lines"] = _dedupe_strings(input_vocabulary_refs)
	affordances["meaningful_interaction_domains"] = _dedupe_strings(meaningful_interaction_domains)
	return affordances
