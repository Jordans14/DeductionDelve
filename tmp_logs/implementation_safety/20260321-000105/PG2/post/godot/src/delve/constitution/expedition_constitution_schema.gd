...
const SEMANTIC_LOCK_SCHEMA_VERSION := 1
...
static func build(
	...
	var semantic_lock_registry := _build_semantic_lock_registry(constitution_hash, str(constitution.get("constitution_id", "")))
	constitution["semantic_lock_registry"] = semantic_lock_registry.duplicate(true)
	compile_metadata["semantic_lock_registry_id"] = str(semantic_lock_registry.get("registry_id", "")).strip_edges()
	compile_metadata["semantic_lock_schema_version"] = int(semantic_lock_registry.get("schema_version", SEMANTIC_LOCK_SCHEMA_VERSION))
	compile_metadata["semantic_lock_tuple_ids"] = _semantic_lock_tuple_ids(Array(semantic_lock_registry.get("tuple_records", [])))
...
static func normalize(raw: Dictionary) -> Dictionary:
	...
	normalized["semantic_lock_registry"] = _build_semantic_lock_registry(
		str(normalized.get("constitution_hash", "")).strip_edges(),
		str(normalized.get("constitution_id", "")).strip_edges()
	)
	var compile_metadata: Dictionary = Dictionary(normalized.get("compile_metadata", {})).duplicate(true)
	if not compile_metadata.is_empty():
		var semantic_lock_registry: Dictionary = Dictionary(normalized.get("semantic_lock_registry", {})).duplicate(true)
		compile_metadata["semantic_lock_registry_id"] = str(semantic_lock_registry.get("registry_id", "")).strip_edges()
		compile_metadata["semantic_lock_schema_version"] = int(semantic_lock_registry.get("schema_version", SEMANTIC_LOCK_SCHEMA_VERSION))
		compile_metadata["semantic_lock_tuple_ids"] = _semantic_lock_tuple_ids(Array(semantic_lock_registry.get("tuple_records", [])))
		normalized["compile_metadata"] = compile_metadata
	return normalized
...
static func semantic_lock_registry(constitution: Dictionary) -> Dictionary:
	var normalized := normalize(constitution)
	return Dictionary(normalized.get("semantic_lock_registry", {})).duplicate(true)

static func validate_semantic_lock_registry(registry: Dictionary) -> Array[String]:
	var failures: Array[String] = []
	if str(registry.get("schema_name", "")).strip_edges() != "SemanticLockRegistry":
		failures.append("semantic lock registry must declare SemanticLockRegistry")
	if int(registry.get("schema_version", 0)) != SEMANTIC_LOCK_SCHEMA_VERSION:
		failures.append("semantic lock registry must remain on schema_version 1")
	var governance_schema := DOCTRINE_SCHEMA_REGISTRY_SCRIPT.governance_schema()
	var required_registry_fields := _string_array(governance_schema.get("semantic_lock_registry_required_fields", []))
	var required_tuple_fields := _string_array(governance_schema.get("semantic_lock_tuple_required_fields", []))
	for field in required_registry_fields:
		if not registry.has(field):
			failures.append("semantic lock registry missing %s" % field)
	var registry_id := str(registry.get("registry_id", "")).strip_edges()
	if registry_id.is_empty():
		failures.append("semantic lock registry must expose registry_id")
	var constitution_hash := str(registry.get("constitution_hash", "")).strip_edges()
	if constitution_hash.is_empty():
		failures.append("semantic lock registry must expose constitution_hash")
	elif constitution_hash != constitution_hash.to_lower():
		failures.append("semantic lock registry constitution_hash must stay lowercase")
	var tuple_records := Array(registry.get("tuple_records", []))
	if tuple_records.is_empty():
		failures.append("semantic lock registry must expose tuple_records")
	var allowed_subject_classes := [
	...
	var allowed_lanes := ["compile", "runtime", "report", "bundle", "persistence", "reload"]
	var allowed_consumers := ["compile", "runtime", "report", "bundle", "persistence", "reload", "tests"]
	var allowed_visibility := ["public", "private", "operator", "bundle_only", "mixed_partitioned"]
	var allowed_derivations := ["copy_only", "subset_copy_public_safe", "digest_only"]
	var seen_tuple_ids: Dictionary = {}
	var seen_field_owners: Dictionary = {}
	var seen_consequence_domain := ""
	for tuple_raw in tuple_records:
		var tuple := Dictionary(tuple_raw)
		var tuple_id := str(tuple.get("tuple_id", "")).strip_edges()
		for field in required_tuple_fields:
			if not tuple.has(field):
				failures.append("semantic lock tuple %s missing %s" % [tuple_id if not tuple_id.is_empty() else "<unknown>", field])
		if tuple_id.is_empty():
			failures.append("semantic lock tuple must expose tuple_id")
			continue
		if seen_tuple_ids.has(tuple_id):
			failures.append("semantic lock tuple %s must be unique" % tuple_id)
		seen_tuple_ids[tuple_id] = true
		var subject_class := str(tuple.get("subject_class", "")).strip_edges()
		if not allowed_subject_classes.has(subject_class):
			failures.append("semantic lock tuple %s declares invalid subject_class %s" % [tuple_id, subject_class])
		var owner_lane := str(tuple.get("owner_lane", "")).strip_edges()
		if not allowed_lanes.has(owner_lane):
			failures.append("semantic lock tuple %s declares invalid owner_lane %s" % [tuple_id, owner_lane])
		var canonical_source_owner := str(tuple.get("canonical_source_owner", "")).strip_edges()
		if canonical_source_owner.is_empty():
			failures.append("semantic lock tuple %s must expose canonical_source_owner" % tuple_id)
		var canonical_source_fields := _string_array(tuple.get("canonical_source_fields", []))
		if canonical_source_fields.is_empty():
			failures.append("semantic lock tuple %s must expose canonical_source_fields" % tuple_id)
		var digest_fields := _string_array(tuple.get("digest_fields", []))
		if digest_fields.is_empty():
			failures.append("semantic lock tuple %s must expose digest_fields" % tuple_id)
		for field_id in digest_fields:
			if not canonical_source_fields.has(field_id):
				failures.append("semantic lock tuple %s digest_fields must remain a subset of canonical_source_fields" % tuple_id)
		for field_id in _string_array(tuple.get("public_projection_fields", [])):
			if not canonical_source_fields.has(field_id):
				failures.append("semantic lock tuple %s public_projection_fields must remain canonical" % tuple_id)
		for field_id in _string_array(tuple.get("private_projection_fields", [])):
			if not canonical_source_fields.has(field_id):
				failures.append("semantic lock tuple %s private_projection_fields must remain canonical" % tuple_id)
		for field_id in _string_array(tuple.get("bundle_only_fields", [])):
			if not canonical_source_fields.has(field_id):
				failures.append("semantic lock tuple %s bundle_only_fields must remain canonical" % tuple_id)
		var alias_read_map: Dictionary = Dictionary(tuple.get("alias_read_map", {})).duplicate(true)
		for alias_key in alias_read_map.keys():
			var alias_target := str(alias_read_map.get(alias_key, "")).strip_edges()
			if alias_target.is_empty() or not canonical_source_fields.has(alias_target):
				failures.append("semantic lock tuple %s alias %s must resolve to a canonical field" % [tuple_id, str(alias_key)])
		var tuple_allowed_consumers := _string_array(tuple.get("allowed_consumers", []))
		if tuple_allowed_consumers.is_empty():
			failures.append("semantic lock tuple %s must expose allowed_consumers" % tuple_id)
		for consumer in tuple_allowed_consumers:
			if not allowed_consumers.has(consumer):
				failures.append("semantic lock tuple %s declares invalid consumer %s" % [tuple_id, consumer])
		var visibility_class := str(tuple.get("visibility_class", "")).strip_edges()
		if not allowed_visibility.has(visibility_class):
			failures.append("semantic lock tuple %s declares invalid visibility_class %s" % [tuple_id, visibility_class])
		var derivation_policy := str(tuple.get("derivation_policy", "")).strip_edges()
		if not allowed_derivations.has(derivation_policy):
			failures.append("semantic lock tuple %s declares invalid derivation_policy %s" % [tuple_id, derivation_policy])
		if not bool(tuple.get("additive_only", false)):
			failures.append("semantic lock tuple %s must remain additive_only" % tuple_id)
		if derivation_policy != "digest_only" and not bool(tuple.get("recompute_forbidden", false)):
			failures.append("semantic lock tuple %s must forbid downstream recompute" % tuple_id)
		var value_domain_map: Dictionary = Dictionary(tuple.get("value_domain_map", {})).duplicate(true)
		for field_id in canonical_source_fields:
			if str(value_domain_map.get(field_id, "")).strip_edges().is_empty():
				failures.append("semantic lock tuple %s missing value domain for %s" % [tuple_id, field_id])
			if seen_field_owners.has(field_id) and str(seen_field_owners.get(field_id, "")).strip_edges() != canonical_source_owner:
				failures.append("semantic lock field %s acquired duplicate authority" % field_id)
			seen_field_owners[field_id] = canonical_source_owner
			if field_id.ends_with(".consequence_event_family"):
				var domain := str(value_domain_map.get(field_id, "")).strip_edges()
				if seen_consequence_domain.is_empty():
					seen_consequence_domain = domain
				elif domain != seen_consequence_domain:
					failures.append("semantic lock consequence_event_family domain drift detected")
		if _string_array(tuple.get("failure_codes", [])).is_empty():
			failures.append("semantic lock tuple %s must expose failure_codes" % tuple_id)
	for subject_class in allowed_subject_classes:
		var tuple_id := "semantic_lock.%s" % subject_class
		if not seen_tuple_ids.has(tuple_id):
			failures.append("semantic lock registry missing tuple %s" % tuple_id)
	return failures
...
static func build_hash(raw_constitution: Dictionary) -> String:
	...
	snapshot.erase("semantic_lock_registry")
...
static func _apply_phase_v3_summary(
	...
	var truth_resolution_envelope: Dictionary = Dictionary(explanation_packet.get("truth_resolution_envelope", {})).duplicate(true)
	...
	current["truth_resolution_envelope_id"] = str(truth_resolution_envelope.get("envelope_id", "")).strip_edges()
	current["truth_resolution_authority_epoch"] = str(truth_resolution_envelope.get("authority_epoch", "")).strip_edges()
	current["truth_resolution_owner_lane"] = str(truth_resolution_envelope.get("owner_lane", "")).strip_edges()
	current["truth_resolution_resolution_class"] = str(truth_resolution_envelope.get("resolution_class", "")).strip_edges()
	current["truth_resolution_contradiction_disposition"] = str(truth_resolution_envelope.get("contradiction_disposition", "")).strip_edges()
	current["truth_resolution_source_digest"] = str(truth_resolution_envelope.get("source_digest", packet_digest)).strip_edges()
	current["truth_resolution_public_summary_refs"] = _string_array(truth_resolution_envelope.get("public_summary_refs", []))
	return current
...
static func _normalize_explanation_packet(raw: Dictionary) -> Dictionary:
	...
	var truth_resolution_envelope: Dictionary = Dictionary(current.get("truth_resolution_envelope", {})).duplicate(true)
	current["truth_resolution_envelope"] = GOVERNANCE_SERVICE_SCRIPT.build_truth_resolution_envelope(
		{
			"packet_id": str(current.get("packet_id", "")).strip_edges(),
			"constitution_hash": str(truth_resolution_envelope.get("source_constitution_hash", current.get("constitution_hash", ""))).strip_edges(),
			"packet_digest": str(current.get("packet_digest", "")).strip_edges(),
			"provenance_source_refs": Array(current.get("provenance_source_refs", [])).duplicate(true)
		},
		{
			"subject_id": str(current.get("packet_id", "")).strip_edges(),
			"subject_kind": str(truth_resolution_envelope.get("subject_kind", "explanation_packet")).strip_edges(),
			"authority_epoch": str(truth_resolution_envelope.get("authority_epoch", "compile_pre_start")).strip_edges(),
			"owner_lane": str(truth_resolution_envelope.get("owner_lane", "compile")).strip_edges(),
			"resolution_class": str(truth_resolution_envelope.get("resolution_class", "authoritative")).strip_edges(),
			"visibility_class": str(truth_resolution_envelope.get("visibility_class", "mixed_partitioned")).strip_edges(),
			"contradiction_disposition": str(truth_resolution_envelope.get("contradiction_disposition", "stable_surface")).strip_edges(),
			"allowed_consumers": Array(truth_resolution_envelope.get("allowed_consumers", ["runtime", "report", "bundle", "persistence", "reload", "tests"])).duplicate(true),
			"derivation_policy": str(truth_resolution_envelope.get("derivation_policy", "subset_copy_public_safe")).strip_edges(),
			"public_summary_refs": Array(truth_resolution_envelope.get("public_summary_refs", ["summary_lines", "immediate", "run", "meta", "public_surface_tags"])).duplicate(true),
			"operator_refs": Array(truth_resolution_envelope.get("operator_refs", ["operator_lines", "provenance_source_refs"])).duplicate(true),
			"failure_codes": Array(truth_resolution_envelope.get("failure_codes", ["runtime_truth_override", "public_operator_ref_leak", "untraceable_summary_field"])).duplicate(true)
		}
	)
	return current

static func _build_semantic_lock_registry(constitution_hash: String, constitution_id: String) -> Dictionary:
	return {
		"schema_name": "SemanticLockRegistry",
		"schema_version": SEMANTIC_LOCK_SCHEMA_VERSION,
		"registry_id": "semantic_lock_%s" % constitution_hash.substr(0, mini(constitution_hash.length(), 12)),
		"constitution_hash": constitution_hash,
		"constitution_id": constitution_id,
		"tuple_records": _semantic_lock_tuple_records()
	}

static func _semantic_lock_tuple_records() -> Array[Dictionary]:
	return [
		_semantic_lock_tuple(
			"constitution_summary",
			"compile",
			"ExpeditionConstitutionSchema.build_runtime_summary",
			[
				"constitution_hash",
				"constitution_id",
				"protocol_state",
			...
			[],
			{
				"doctrine": "constitution_summary.doctrine_label"
			},
			["compile", "runtime", "report", "bundle", "persistence", "reload", "tests"],
			"public",
			"subset_copy_public_safe",
			{
				"constitution_hash": "md5_digest",
				"constitution_id": "constitution_id",
				"protocol_state": "protocol_state",
				"doctrine_family": "doctrine_family_id",
				"doctrine_label": "public_label",
				"pressure_line": "public_pressure_line",
				"world_goal": "public_goal_line",
				"dominant_minds": "mind_id_list",
				"dominant_forces": "force_tag_list",
				"dominant_domains": "domain_tag_list",
				"pressure_grammar": "pressure_grammar_tags",
				"symbolic_motifs": "symbolic_motif_tags",
				"archive_tone": "archive_tone",
				"item_ecology_bias": "ecology_bias_label",
				"group_tension_bias": "group_tension_label",
				"convergence_axis": "convergence_axis_label"
			},
			[
				"constitution_summary_missing",
				"constitution_summary_projection_drift",
				"constitution_summary_duplicate_authority"
			]
		),
		_semantic_lock_tuple(
			"provenance_contract",
			"compile",
			"GovernanceService.build_explanation_packet",
			[
			...
			"subset_copy_public_safe",
			{
				...
				"explanation_packet_digest": "md5_digest",
				"public_trace_classes": "public_trace_class_list",
				"private_trace_classes": "private_trace_class_list",
				"public_surface_tags": "public_surface_tag_list",
				"provenance_source_refs": "operator_reference_list"
			},
			[
				"provenance_contract_missing",
				"provenance_projection_leak",
				"provenance_digest_drift"
			]
		),
		_semantic_lock_tuple(
			"combo_contract",
			"runtime",
			"ItemSynergyService._build_combo_contract",
			[
				"combo_contract_version",
				"combo_contract_digest",
				"combo_family_ids",
				"combo_pressure_tags",
				"public_surface_tags",
				"combo_entries"
			...
			[],
			{
				"combo_public_surface_tags": "combo_contract.public_surface_tags"
			},
			["runtime", "report", "bundle", "persistence", "reload", "tests"],
			"mixed_partitioned",
			"subset_copy_public_safe",
			{
				"combo_contract_version": "contract_version",
				"combo_contract_digest": "md5_digest",
				"combo_family_ids": "combo_family_id_list",
				"combo_pressure_tags": "combo_pressure_tag_list",
				"public_surface_tags": "public_surface_tag_list",
				"combo_entries": "private_combo_entry_list"
			},
			[
				"combo_contract_missing",
				"combo_projection_leak",
				"combo_digest_drift"
			]
		),
		_semantic_lock_tuple(
			"artifact_consequence",
			"runtime",
			"ArtifactService.build_consequence_contract",
			[
			...
			["social_hook_tags", "encounter_hook_tags"],
			[],
			{},
			["runtime", "report", "bundle", "persistence", "reload", "tests"],
			"mixed_partitioned",
			"subset_copy_public_safe",
			{
				"artifact_consequence_version": "contract_version",
				"public_consequence_tags": "public_consequence_tag_list",
				"consequence_event_family": "canonical_consequence_event_family",
				"burden_band": "burden_band",
				"valuation_band": "valuation_band",
				"return_consequence_state": "return_state",
				"market_regime_id": "market_regime_id",
				"market_carrier_risk_band": "carrier_risk_band",
				"custody_chain_summary": "custody_summary",
				"social_hook_tags": "private_social_hook_tag_list",
				"encounter_hook_tags": "private_encounter_hook_tag_list"
			},
			[
				"artifact_consequence_missing",
				"artifact_projection_leak",
				"artifact_consequence_event_family_drift"
			]
		),
		_semantic_lock_tuple(
			"social_consequence",
			"runtime",
			"GameController._build_social_consequence_summary",
			[
				"social_consequence_version",
			...
			"subset_copy_public_safe",
			{
				"social_consequence_version": "contract_version",
				"public_evidence_tags": "public_evidence_tag_list",
				"private_evidence_tags": "private_evidence_tag_list",
				"witness_pressure": "witness_pressure_band",
				"counterfeit_pressure": "counterfeit_pressure_band",
				"relationship_pressure": "relationship_pressure_band",
				"blame_surface_tags": "public_blame_tag_list",
				"consequence_read_refs": "operator_reference_list"
			},
			[
				"social_consequence_missing",
				"social_projection_leak",
				"social_consequence_drift"
			]
		),
		_semantic_lock_tuple(
			"encounter_apex_consequence",
			"report",
			"GameController._build_encounter_apex_consequence_summary",
			[
			...
			["aftermath_consequence_refs"],
			{},
			["report", "bundle", "persistence", "reload", "tests"],
			"mixed_partitioned",
			"subset_copy_public_safe",
			{
				"encounter_apex_consequence_version": "contract_version",
				"encounter_resolution_state": "encounter_resolution_state",
				"apex_resolution_state": "apex_resolution_state",
				"anchored_pressures": "pressure_tag_list",
				"consequence_classes": "consequence_class_list",
				"local_aftermath_tags": "local_aftermath_tag_list",
				"world_aftermath_tags": "world_aftermath_tag_list",
				"aftermath_consequence_refs": "operator_reference_list"
			},
			[
				"encounter_apex_consequence_missing",
				"encounter_apex_projection_leak",
				"encounter_apex_drift"
			]
		),
		_semantic_lock_tuple(
			"lifecycle_surface",
			"compile",
			"ExpeditionConstitutionSchema._apply_phase3_market_summary",
			[
				"active_regime_ids",
				"lifecycle_state_ids",
				"market_regime_lines",
			...
			"subset_copy_public_safe",
			{
				"active_regime_ids": "market_regime_id_list",
				"lifecycle_state_ids": "lifecycle_state_id_list",
				"market_regime_lines": "public_market_line_list",
				"lifecycle_lines": "public_lifecycle_line_list",
				"market_regime_id": "market_regime_id",
				"market_regime_family": "market_regime_family",
				"market_prestige_band": "market_prestige_band",
				"market_carrier_risk_band": "carrier_risk_band"
			},
			[
				"lifecycle_surface_missing",
				"lifecycle_surface_drift",
				"lifecycle_surface_duplicate_authority"
			]
		),
		_semantic_lock_tuple(
			"governance_hook_set",
			"runtime",
			"GovernanceService.build_governance_hook_set",
			[
				"activation_epoch",
				"active_channels",
				"dormant_channels",
			...
			[],
			{},
			["runtime", "bundle", "persistence", "reload", "tests"],
			"operator",
			"copy_only",
			{
				"activation_epoch": "activation_epoch",
				"active_channels": "governance_channel_list",
				"dormant_channels": "governance_channel_list",
				"safe_mode_active": "boolean_flag",
				"quarantine_ids": "quarantine_id_list",
				"packet_id": "packet_id",
				"packet_schema_version": "packet_version",
				"packet_digest": "md5_digest",
				"fairness_flags": "fairness_flag_list",
				"priority_channels": "priority_channel_list",
				"available_actions": "governance_action_list",
				"trigger_slots": "governance_trigger_map",
				"resurrection_candidates": "resurrection_candidate_list",
				"summary_lines": "operator_summary_lines"
			},
			[
				"governance_hook_set_missing",
				"governance_hook_projection_drift",
				"governance_hook_duplicate_authority"
			]
		),
		_semantic_lock_tuple(
			"onboarding_public_safe",
			"persistence",
			"ProfileService.apply_run_record",
			[
				"explanation_packet_lines",
				"review_surface_lines",
				"signal_budget_lines",
			...
			"subset_copy_public_safe",
			{
				"explanation_packet_lines": "public_summary_line_list",
				"review_surface_lines": "public_review_line_list",
				"signal_budget_lines": "public_budget_line_list",
				"public_consequence_tags": "public_consequence_tag_list",
				"return_consequence_state": "return_state",
				"burden_band": "burden_band",
				"valuation_band": "valuation_band",
				...
				"reentry_hook_id": "reentry_hook_id"
			},
			[
				"onboarding_public_safe_missing",
				"onboarding_public_safe_drift",
				"onboarding_public_safe_projection_leak"
			]
		)
	]

static func _semantic_lock_tuple(
	subject_class: String,
	owner_lane: String,
	canonical_source_owner: String,
	canonical_field_suffixes: Array,
	digest_field_suffixes: Array,
	public_field_suffixes: Array,
	private_field_suffixes: Array,
	bundle_only_field_suffixes: Array,
	alias_read_map: Dictionary,
	allowed_consumers: Array,
	visibility_class: String,
	derivation_policy: String,
	value_domain_suffixes: Dictionary,
	failure_codes: Array
) -> Dictionary:
	var field_prefix := "%s." % subject_class
	return {
		"tuple_id": "semantic_lock.%s" % subject_class,
		"subject_class": subject_class,
		"owner_lane": owner_lane,
		"canonical_source_owner": canonical_source_owner,
		"canonical_source_fields": _semantic_lock_prefix_fields(field_prefix, canonical_field_suffixes),
		"digest_fields": _semantic_lock_prefix_fields(field_prefix, digest_field_suffixes),
		"public_projection_fields": _semantic_lock_prefix_fields(field_prefix, public_field_suffixes),
		"private_projection_fields": _semantic_lock_prefix_fields(field_prefix, private_field_suffixes),
		"bundle_only_fields": _semantic_lock_prefix_fields(field_prefix, bundle_only_field_suffixes),
		"alias_read_map": _semantic_lock_alias_map(subject_class, alias_read_map),
		"allowed_consumers": _string_array(allowed_consumers),
		"visibility_class": visibility_class,
		"derivation_policy": derivation_policy,
		"additive_only": true,
		"recompute_forbidden": derivation_policy != "digest_only",
		"value_domain_map": _semantic_lock_value_domain_map(field_prefix, value_domain_suffixes),
		"failure_codes": _string_array(failure_codes)
	}

static func _semantic_lock_prefix_fields(field_prefix: String, suffixes: Array) -> Array[String]:
	var result: Array[String] = []
	for suffix_raw in suffixes:
		var suffix := str(suffix_raw).strip_edges()
		if suffix.is_empty():
			continue
		result.append("%s%s" % [field_prefix, suffix])
	return _string_array(result)

static func _semantic_lock_alias_map(subject_class: String, aliases: Dictionary) -> Dictionary:
	var result := {}
	for legacy_key in aliases.keys():
		var canonical_field := str(aliases.get(legacy_key, "")).strip_edges()
		if canonical_field.is_empty():
			continue
		result[str(legacy_key).strip_edges()] = canonical_field if canonical_field.find(".") != -1 else "%s.%s" % [subject_class, canonical_field]
	return result

static func _semantic_lock_value_domain_map(field_prefix: String, value_domains: Dictionary) -> Dictionary:
	var result := {}
	for suffix_raw in value_domains.keys():
		var suffix := str(suffix_raw).strip_edges()
		if suffix.is_empty():
			continue
		result["%s%s" % [field_prefix, suffix]] = str(value_domains.get(suffix_raw, "")).strip_edges()
	return result

static func _semantic_lock_tuple_ids(tuple_records: Array) -> Array[String]:
	var ids: Array[String] = []
	for tuple_raw in tuple_records:
		var tuple_id := str(Dictionary(tuple_raw).get("tuple_id", "")).strip_edges()
		if not tuple_id.is_empty() and not ids.has(tuple_id):
			ids.append(tuple_id)
	return ids
